...
1. Write or copy into a **staged file** first
2. Validate the staged file before final placement (where validation is available)
3. Replace the final path atomically with `os.replace` when the filesystem supports it
4. For cross-filesystem moves, copy into a hidden partial file in the destination directory, then atomically replace
