"""Test-suite helpers for HTTP integration tests that spin ``create_app`` with ``WEIR_HOME``.
...
def integration_test_quiesce_periodic_enqueue(monkeypatch: pytest.MonkeyPatch) -> None:
    """Turn off :class:`weir.core.config.WeirSettings` periodic enqueue toggles for the suite DB."""

    monkeypatch.setenv("WEIR_REFINER_WATCHED_FOLDER_REMUX_SCAN_DISPATCH_SCHEDULE_ENABLED", "0")
    monkeypatch.setenv("WEIR_REFINER_WORK_TEMP_STALE_SWEEP_MOVIE_SCHEDULE_ENABLED", "0")
    monkeypatch.setenv("WEIR_REFINER_WORK_TEMP_STALE_SWEEP_TV_SCHEDULE_ENABLED", "0")
    monkeypatch.setenv("WEIR_REFINER_MOVIE_FAILURE_CLEANUP_SCHEDULE_ENABLED", "0")
    monkeypatch.setenv("WEIR_REFINER_TV_FAILURE_CLEANUP_SCHEDULE_ENABLED", "0")
