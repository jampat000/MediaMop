...
from weir.refiner import refiner_remux_mux
...
def test_ffprobe_success_diagnostics_are_debug_not_warning(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(refiner_remux_mux, "resolve_ffprobe_ffmpeg", lambda *, weir_home: ("ffprobe", "ffmpeg"))
    monkeypatch.setattr(
    ...
    refiner_remux_mux.ffprobe_json(media, weir_home=str(tmp_path))
