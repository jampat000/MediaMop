"""The pause endpoint — ``/api/v1/pause``, the one pause switch (#337, #459)."""
...
def test_processing_starts_unpaused_and_says_what_a_pause_would_do(signed_in: TestClient) -> None:
    body = auth_get(signed_in, "/api/v1/pause").json()
...
def test_a_pause_change_without_a_csrf_token_is_refused(signed_in: TestClient) -> None:
    response = auth_put(signed_in, "/api/v1/pause", json={"paused": True, "scan_while_paused": True})
