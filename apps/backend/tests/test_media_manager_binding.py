...
from weir.platform.arr_library.arr_connection_crypto import encrypt_arr_api_key
from weir.platform.media_managers.connection_model import MediaManagerConnectionRow
from weir.platform.media_managers.manager_binding import (
...
_BINDING = "weir.platform.media_managers.manager_binding"


@pytest.fixture
def settings(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> WeirSettings:
    monkeypatch.setenv("WEIR_HOME", str(tmp_path))
    monkeypatch.setenv("WEIR_SESSION_SECRET", "session-secret-abcdefghijklmnopqrstuvwxyz")
    monkeypatch.setenv("WEIR_CREDENTIALS_SECRET", "credentials-secret")
    monkeypatch.delenv("WEIR_ARR_RADARR_BASE_URL", raising=False)
    monkeypatch.delenv("WEIR_ARR_RADARR_API_KEY", raising=False)
    monkeypatch.delenv("WEIR_ARR_SONARR_BASE_URL", raising=False)
    monkeypatch.delenv("WEIR_ARR_SONARR_API_KEY", raising=False)
...
def test_a_library_can_be_served_by_more_than_one_connection(session: Session, settings: WeirSettings) -> None:
...
def test_a_manager_serving_both_scopes_answers_for_both(session: Session, settings: WeirSettings) -> None:
...
def test_radarr_and_deluno_are_both_consulted_for_movies(session: Session, settings: WeirSettings) -> None:
...
def test_a_sonarr_connection_is_not_asked_about_movies(session: Session, settings: WeirSettings) -> None:
...
def test_a_disabled_connection_is_not_consulted(session: Session, settings: WeirSettings) -> None:
    _add(session, settings, kind="radarr", name="Off", enabled=False)
    assert connections_for_scope(session, settings, media_scope="movie") == []


def test_a_connection_with_no_saved_key_cannot_be_asked(session: Session, settings: WeirSettings) -> None:
...
def test_environment_credentials_still_work_when_nothing_claims_the_scope(
    ...
    monkeypatch.setenv("WEIR_HOME", str(tmp_path))
    monkeypatch.setenv("WEIR_ARR_RADARR_BASE_URL", "http://radarr.local")
    monkeypatch.setenv("WEIR_ARR_RADARR_API_KEY", "env-key")
    env_settings = WeirSettings.load()
