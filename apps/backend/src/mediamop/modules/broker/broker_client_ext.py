"""EXT.to RSS native indexer client."""
...
class BrokerClientExt(BrokerClientBase):
    slug = "native__ext"
    protocol = "torrent"
    display_name = "EXT"
    ...
    async def search(
        ...
        for it in items:
            ...
            if not title or not link:
                continue
            mag = link if link.startswith("magnet:") else None
    ...
    async def test(self, api_key: str = "", base_url: str = "") -> bool:
        _ = api_key, base_url
        raw = await self._get_bytes("https://ext.to/rss/?q=test")
