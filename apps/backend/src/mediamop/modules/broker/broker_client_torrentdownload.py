"""TorrentDownload.info RSS native indexer client."""
...
class BrokerClientTorrentdownload(BrokerClientBase):
    slug = "native__torrentdownload"
    protocol = "torrent"
    display_name = "TorrentDownload"
    ...
    async def search(
        ...
        url = f"https://www.torrentdownload.info/rss.xml?q={q}"
    ...
    async def test(self, api_key: str = "", base_url: str = "") -> bool:
        _ = api_key, base_url
        raw = await self._get_bytes("https://www.torrentdownload.info/rss.xml?q=test")
