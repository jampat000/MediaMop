"""AnimeTosho RSS native indexer client."""
...
class BrokerClientAnimetosho(BrokerClientBase):
    slug = "native__animetosho"
    protocol = "torrent"
    ...
    async def search(
        ...
        url = f"https://feed.animetosho.org/rss2?q={q}"
        ...
        for it in items:
            ...
            enc_url = str(it.get("enclosure_url") or "").strip()
            enc_type = str(it.get("enclosure_type") or "").lower()
            url_final = enc_url or link
            if not title or not url_final:
                continue
            proto = "usenet" if "nzb" in enc_type else "torrent"
    ...
    async def test(self, api_key: str = "", base_url: str = "") -> bool:
        _ = api_key, base_url
        raw = await self._get_bytes("https://feed.animetosho.org/rss2?q=test")
        return raw is not None and (b"<rss" in raw[:4000].lower() or b"feed" in raw[:200].lower())
