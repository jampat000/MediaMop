"""ShanaProject RSS native indexer client (client-side title filter)."""
...
class BrokerClientShanaproject(BrokerClientBase):
    slug = "native__shanaproject"
    protocol = "torrent"
    display_name = "ShanaProject"
    ...
    async def search(
        ...
        url = "https://www.shanaproject.com/feeds/current/"
    ...
    async def test(self, api_key: str = "", base_url: str = "") -> bool:
        _ = api_key, base_url
        raw = await self._get_bytes("https://www.shanaproject.com/feeds/current/")
