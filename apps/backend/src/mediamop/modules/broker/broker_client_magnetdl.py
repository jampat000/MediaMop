"""MagnetDL RSS native indexer client."""
...
class BrokerClientMagnetdl(BrokerClientBase):
    slug = "native__magnetdl"
    protocol = "torrent"
    ...
    async def search(
        ...
        q = (query.strip() or "a").lower()
        first = q[0] if q else "a"
        qpath = urllib.parse.quote(q, safe="")
        url = f"https://www.magnetdl.com/{first}/{qpath}/se/desc/rss/"
    ...
    async def test(self, api_key: str = "", base_url: str = "") -> bool:
        _ = api_key, base_url
        raw = await self._get_bytes("https://www.magnetdl.com/t/test/se/desc/rss/")
        return raw is not None and (b"<rss" in raw[:4000].lower() or b"<?xml" in raw[:200].lower())
