"""Broker-local persisted job queue — coordination for in-process Broker workers only."""
...
class BrokerJobStatus(str, enum.Enum):
    """Persisted in ``broker_jobs.status`` (VARCHAR)."""
...
class BrokerJob(Base):
    """One row of durable Broker work — claim/lease before execution."""
