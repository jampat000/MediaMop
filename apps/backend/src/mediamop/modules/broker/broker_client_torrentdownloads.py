"""TorrentDownloads RSS native indexer client."""
...
class BrokerClientTorrentdownloads(BrokerClientBase):
    slug = "native__torrentdownloads"
    protocol = "torrent"
    display_name = "TorrentDownloads"
    ...
    async def search(
        ...
        url = f"https://www.torrentdownloads.pro/rss.xml?search={q}"
    ...
    async def test(self, api_key: str = "", base_url: str = "") -> bool:
        _ = api_key, base_url
        raw = await self._get_bytes("https://www.torrentdownloads.pro/rss.xml?search=test")
