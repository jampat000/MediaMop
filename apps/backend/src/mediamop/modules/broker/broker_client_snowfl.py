"""Snowfl RSS native indexer client."""
...
class BrokerClientSnowfl(BrokerClientBase):
    slug = "native__snowfl"
    protocol = "torrent"
    display_name = "Snowfl"
    ...
    async def search(
        ...
        qenc = urllib.parse.quote(query.strip() or " ", safe="")
        url = f"https://snowfl.com/{qenc}/0/0/SEED/1/rss"
    ...
    async def test(self, api_key: str = "", base_url: str = "") -> bool:
        _ = api_key, base_url
        raw = await self._get_bytes("https://snowfl.com/test/0/0/SEED/1/rss")
