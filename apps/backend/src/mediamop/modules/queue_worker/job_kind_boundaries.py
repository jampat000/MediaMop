...
# Pruner moved to Deluno too (#473), and its prefix is retired the same way: a
# ``pruner.*`` row left by an older install is refused by every lane rather than picked
# up by a worker that no longer exists.
LEGACY_PRUNER_QUEUE_JOB_KIND_PREFIX = "pruner."
...
def _legacy_or_foreign_prefixes() -> tuple[str, ...]:
    return (
        ...
        LEGACY_PRUNER_QUEUE_JOB_KIND_PREFIX,
...
_FORBIDDEN_ON_REFINER_LANE: tuple[str, ...] = (*_legacy_or_foreign_prefixes(),)
...
def validate_refiner_enqueue_job_kind(job_kind: str) -> None:
    """Refiner queue rows must use the Refiner lane only (no retired or foreign namespace)."""
