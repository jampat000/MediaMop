"""Atomic claim / lease / complete / fail for :class:`~mediamop.refiner.jobs_model.RefinerJob`.
...
from mediamop.refiner.job_kind_guard import validate_refiner_enqueue_job_kind
...
def _admission_predicate(admission: object | None) -> tuple[str, dict[str, object]]:
    ...
    if pause is not None and getattr(pause, "paused", False):
        from mediamop.refiner.refiner_work_admission import DETECTION_JOB_KIND_PREFIXES
