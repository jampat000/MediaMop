"""Refiner — the MediaMop application: media refinement for movies and TV.
...
(``MEDIAMOP_REFINER_WORKER_COUNT``). Composition may inject neutral ports.

Shipped durable ``refiner.*`` families include queue evaluation, candidate gate, and
``refiner.file.remux_pass.v1`` (ffprobe + remux planning under ``mediamop.refiner.refiner_remux_*``;
...
from mediamop.refiner.queue_adapter import (
    MOVIE_QUEUE_DIALECT,
    TV_QUEUE_DIALECT,
    QueueDialect,
...
from mediamop.refiner.queue_row_plumbing import normalize_storage_path
