"""Headless server entry point for the Weir Windows package.

Launched by the .NET tray app via ``WeirServer.exe --serve --port <port>``.
...
def _prepare_environment(resource_root: Path, runtime_home: Path) -> None:
    ...
    if not (web_dist / "index.html").is_file():
        raise RuntimeError("Bundled web assets are missing from the Weir desktop package.")
    runtime_home.mkdir(parents=True, exist_ok=True)
    os.environ.setdefault("WEIR_ENV", "production")
    os.environ["WEIR_HOME"] = str(runtime_home)
    os.environ["WEIR_WEB_DIST"] = str(web_dist)
    os.environ["WEIR_ALEMBIC_ROOT"] = str(resource_root)
    ...
    os.environ["WEIR_SESSION_SECRET"] = _ensure_session_secret(runtime_home)
...
def _run_migrations(resource_root: Path) -> None:
    ...
    if not alembic_ini.is_file() or not alembic_dir.is_dir():
        raise RuntimeError("Bundled migration assets are missing from the Weir desktop package.")
