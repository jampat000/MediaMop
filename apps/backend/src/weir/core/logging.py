...
from weir.platform.http.request_context import current_job_id, current_request_id
from weir.platform.metrics.service import record_log_record
...
class WeirLogFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        record.correlation_id = current_request_id()
        record.job_id = current_job_id()
        if not getattr(record, "_weir_metrics_counted", False):
            record_log_record(record.levelname)
            record._weir_metrics_counted = True
        return True
...
def configure_logging(settings: WeirSettings) -> None:
    ...
    log_path = Path(settings.log_dir) / "weir.log"
