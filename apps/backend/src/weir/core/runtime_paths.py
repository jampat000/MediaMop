"""SQLite-first runtime directories under ``WEIR_HOME``.

Default layout (when overrides are unset):

- ``{home}/data/weir.sqlite3`` — database file (via ``WEIR_DB_PATH`` default)
- ``{home}/backups/``
- ``{home}/logs/``
- ``{home}/temp/``
"""
...
from weir.core.paths import resolve_weir_home
...
def resolve_db_path(home: Path) -> Path:
    ...
    override = _env_path("WEIR_DB_PATH")
    if override:
        p = Path(override).expanduser()
        if not p.is_absolute():
            return (home / p).resolve()
        return p.resolve()
    return (home / "data" / "weir.sqlite3").resolve()


def resolve_backup_dir(home: Path) -> Path:
    override = _env_path("WEIR_BACKUP_DIR")
    if override:
        p = Path(override).expanduser()
        if not p.is_absolute():
            return (home / p).resolve()
...
def resolve_log_dir(home: Path) -> Path:
    override = _env_path("WEIR_LOG_DIR")
    if override:
        p = Path(override).expanduser()
        if not p.is_absolute():
            return (home / p).resolve()
...
def resolve_temp_dir(home: Path) -> Path:
    override = _env_path("WEIR_TEMP_DIR")
    if override:
        p = Path(override).expanduser()
        if not p.is_absolute():
            return (home / p).resolve()
...
def assert_sqlite_db_location_usable(db_path: Path) -> None:
    ...
    - Rejects ``WEIR_DB_PATH`` resolving to an existing **directory** (common misconfiguration).
