...
Tokens are **signed** (``itsdangerous.TimestampSigner``) with ``WEIR_SESSION_SECRET``.
...
(``WEIR_TRUSTED_BROWSER_ORIGINS`` if set, otherwise ``WEIR_CORS_ORIGINS``). In
``WEIR_ENV=development``, each configured ``http(s)://localhost`` / ``127.0.0.1`` origin is
...
def current_raw_session_token(request: Request, settings: WeirSettings) -> str | None:
...
def validate_browser_post_origin(request: Request, settings: WeirSettings) -> None:
...
def require_session_secret(settings: WeirSettings) -> str:
    s = (settings.session_secret or "").strip()
    if not s:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="WEIR_SESSION_SECRET must be set for auth endpoints.",
        )
    return s
