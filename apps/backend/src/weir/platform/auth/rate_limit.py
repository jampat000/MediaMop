...
Counters live in process memory and are valid only under Weir's supported
...
def client_rate_limit_key(request) -> str:
    ...
    through ``WEIR_TRUSTED_PROXY_IPS``.
    """
...
def _parse_proxy_network(raw: str):
    try:
        ...
        logger.warning("Ignoring invalid WEIR_TRUSTED_PROXY_IPS entry.")
        return None
