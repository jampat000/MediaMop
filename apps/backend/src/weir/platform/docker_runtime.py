...
def load_docker_ownership_plan(
    ...
    puid = _parse_non_negative_int(
        _first_env(values, "WEIR_PUID", "PUID") or "1000",
        name="WEIR_PUID",
    )
    pgid = _parse_non_negative_int(
        _first_env(values, "WEIR_PGID", "PGID") or "1000",
        name="WEIR_PGID",
    )
    return DockerOwnershipPlan(
        ...
        chown_watched=_parse_boolish(
            values.get("WEIR_CHOWN_WATCHED"),
            name="WEIR_CHOWN_WATCHED",
            default=False,
        ),
        chown_temp=_parse_boolish(
            values.get("WEIR_CHOWN_TEMP"),
            name="WEIR_CHOWN_TEMP",
            default=False,
        ),
        chown_output=_parse_boolish(
            values.get("WEIR_CHOWN_OUTPUT"),
            name="WEIR_CHOWN_OUTPUT",
            default=False,
        ),
        watched_dir_mode=_parse_directory_mode(
            values.get("WEIR_DIR_MODE_WATCHED"),
            name="WEIR_DIR_MODE_WATCHED",
        ),
        temp_dir_mode=_parse_directory_mode(
            values.get("WEIR_DIR_MODE_TEMP"),
            name="WEIR_DIR_MODE_TEMP",
        ),
        output_dir_mode=_parse_directory_mode(
            values.get("WEIR_DIR_MODE_OUTPUT"),
            name="WEIR_DIR_MODE_OUTPUT",
        ),
    )
