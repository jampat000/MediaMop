...
from weir.platform.notifications.model import NotificationChannel
from weir.platform.notifications.ops import get_channels_for_event
from weir.platform.outbound_http import post_json_to_external_url, safe_external_error_message
...
def _is_permanently_failed(session: Session, module: str, job_id: int) -> bool:
    ...
    safe_module = module if module in {"refiner"} else None
...
def test_dispatch_notification_channel(
    ...
    *,
    ...
    try:
        _post_one(
            channel,
            title="Weir test notification",
            detail="This is a test notification from Weir.",
            event=event,
            module=module,
            job_id=job_id,
            job_kind=job_kind,
