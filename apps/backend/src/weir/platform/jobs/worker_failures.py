...
from weir.platform.observability.failure_messages import OperatorFailure, operator_failure_from_exception
...
def job_failure(*, module: str, exc: BaseException, will_retry: bool) -> OperatorFailure:
    return operator_failure_from_exception(
        ...
        exc=exc,
        continuation=(
            "Weir will try this job again shortly."
            if will_retry
...
def refused_job_error(*, module: str, technical_reason: str, will_retry: bool) -> str:
    """A job this worker cannot run, e.g. one queued for another part of Weir."""
