    ...
    *,
    ...
    if status == "pending":
        return JobOperatorStatus(
            ...
            next_action="No action is needed. Weir will start it when the required worker capacity is available.",
    ...
    if status == "handler_ok_finalize_failed":
        return JobOperatorStatus(
            operator_message=(f"The {label} work completed{subject}, but Weir could not finish saving the job result."),
            next_action="Use Recover result below. Weir will not run the media work again.",
            technical_detail=technical,
        )
    ...
    if "database is locked" in lower or "database table is locked" in lower:
        if module.lower() == "refiner":
            return JobOperatorStatus(
                operator_message=(
                    f"Weir could not save the Refiner result while another local operation was using the database{subject}."
                ),
                next_action=(
        ...
        return JobOperatorStatus(
            operator_message=f"Weir could not save the {label} result because another local operation was using the database.",
    ...
    if "could not find this file" in lower or "file not found" in lower or "no such file" in lower:
        return JobOperatorStatus(
            operator_message=f"Weir could not find this file under the saved watched folder{subject}.",
    ...
    if "modified too recently" in lower or "still being written" in lower:
        return JobOperatorStatus(
            operator_message=f"Weir is waiting for this file to finish changing{subject}.",
