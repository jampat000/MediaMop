...
from weir.platform.suite_settings.schemas import (
    SuiteUpdateStatusOut,
    UpdateSettingsOut,
    UpdateStateOut,
)
from weir.version import __version__

logger = logging.getLogger(__name__)

DOCKER_IMAGE = "ghcr.io/jampat000/weir"


def _detect_install_type() -> str:
    runtime = (os.environ.get("WEIR_RUNTIME") or "").strip().lower()
...
def _build_release_status(
    *,
    ...
    try:
        ...
        if exc.response.status_code == 404:
            return SuiteUpdateStatusOut(
                ...
                summary="No public Weir release is published yet.",
    ...
    if not release.version:
        ...
        summary = f"Weir {release.version} is available."
    else:
        status = "up_to_date"
        summary = f"This install is already on Weir {current_version}."
    ...
    return SuiteUpdateStatusOut(
        ...
        in_app_upgrade_summary=(
            "Updates are managed by the Weir desktop app via Velopack." if install_type == "windows" else None
        ),
    )


def build_suite_update_status(
    settings: WeirSettings | None = None,
) -> SuiteUpdateStatusOut:
...
def get_update_settings(settings: WeirSettings) -> UpdateSettingsOut:
    path = Path(settings.weir_home) / _UPDATE_SETTINGS_FILE
    if not path.exists():
...
def get_update_state(settings: WeirSettings) -> UpdateStateOut:
    path = Path(settings.weir_home) / _UPDATE_STATE_FILE
    if not path.exists():
...
def write_apply_update_flag(settings: WeirSettings) -> None:
    path = Path(settings.weir_home) / _APPLY_UPDATE_FLAG
    path.touch()
