"""Canonical GitHub release metadata helpers for Weir updates."""
...
WINDOWS_INSTALLER_ASSET_NAME = "Weir-win-Setup.exe"
LEGACY_WINDOWS_INSTALLER_ASSET_NAME = "WeirSetup.exe"
...
def release_headers(user_agent_version: str) -> dict[str, str]:
    return {
        ...
        "User-Agent": f"Weir/{user_agent_version}",
...
def fetch_release_record_by_version(
    ...
    *,
    ...
    if record.tag_name != tag:
        raise ValueError(f"Release tag mismatch for Weir update: expected {tag}, got {record.tag_name}.")
    if record.draft or record.prerelease:
        raise ValueError(f"Weir update {tag} is not a stable published release.")
    return record
