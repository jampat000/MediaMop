...
from weir.platform.configuration_bundle.service import build_configuration_bundle
...
def create_suite_configuration_backup(session: Session, *, settings: WeirSettings) -> SuiteConfigurationBackupRow:
    backup_root = _backup_dir(settings)
...
def _prune_old_backups(session: Session, *, settings: WeirSettings) -> None:
    rows = list(
        session.scalars(
            select(SuiteConfigurationBackupRow).order_by(
                SuiteConfigurationBackupRow.created_at.desc(),
