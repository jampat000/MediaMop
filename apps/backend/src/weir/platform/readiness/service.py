...
from weir.platform.health.service import database_is_connected
from weir.platform.jobs.worker_health import build_worker_health_snapshot
from weir.platform.readiness.schemas import ReadinessResponse, ReadinessStep, ReadinessWorkerOut
from weir.refiner.refiner_watcher_state import watcher_summary
    ...
    steps = [
        ReadinessStep(
            ...
            detail=(
                ...
                if database_ready
                else "Weir could not verify the local database connection."
                if startup_complete
                else "Weir is preparing the local database."
            ),
        ),
        ReadinessStep(
            name="workers",
            ...
            detail=(
                ...
                if startup_complete
                else "Weir is starting background workers and schedules."
            ),
        ),
    ]
