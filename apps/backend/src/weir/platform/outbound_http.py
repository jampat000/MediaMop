...
def resolve_public_external_endpoint(raw: str) -> ResolvedExternalEndpoint:
    ...
    try:
        ...
        raise ExternalEndpointError("Weir could not resolve the notification destination.") from exc
    addresses: list[str] = []
    ...
    *,
    ...
    try:
        try:
            ...
            raise ExternalEndpointError("Weir could not connect to the notification destination.") from exc
        ...
        if endpoint.scheme == "https":
            ...
            try:
                ...
                raise ExternalEndpointError("Weir could not establish secure notification delivery.") from exc
        ...
        raise ExternalEndpointError("Weir could not deliver the notification.") from exc
    finally:
...
def safe_external_error_message(exc: BaseException) -> str:
    ...
    if isinstance(exc, ExternalEndpointError):
        return str(exc)
    return "Weir could not deliver the notification. Check the destination and server logs."
