...
from weir.core.credentials_rotation import credential_secret_candidates
...
def _active_fernet(settings: WeirSettings) -> tuple[Fernet | None, Literal["credentials:v1", "session-legacy:v1"]]:
...
def _legacy_fernet(settings: WeirSettings) -> Fernet | None:
...
def encrypt_arr_api_key(settings: WeirSettings, plaintext: str) -> str:
    """Encrypt API key for SQLite storage."""

    f, key_id = _active_fernet(settings)
    if f is None:
        msg = "Cannot save library API keys until WEIR_CREDENTIALS_SECRET or WEIR_SESSION_SECRET is set."
...
def decrypt_arr_api_key(settings: WeirSettings, ciphertext: str) -> str | None:
...
def rewrap_arr_api_key(settings: WeirSettings, ciphertext: str) -> str | None:
