...
- **``outputPath`` is a path on the manager's host.** Deluno refuses a completion whose
  ...
  not necessarily how Weir sees it (containers, mapped drives). The path is rebuilt
...
def build_completion_body(
    *,
    ...
    if succeeded:
        ...
        if rejected:
            # The opt-in reject policy: Weir removes the download once this is accepted, and
...
def record_handoff_report(
    ...
    *,
    ...
    if not delivery.accepted:
        title = f"Weir could not tell {name} about {file_name}"
        ...
        title = f"Told {name} that Weir could not process {file_name}"
