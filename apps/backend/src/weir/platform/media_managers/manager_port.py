"""The three questions Weir asks a media manager, and the answers it accepts.
...
``queue_rows()``
    ...
    :func:`~weir.refiner.queue_adapter.map_queue_row_to_refiner_view` can
...
# ``display_name`` because that one spells out the payload shape ("Generic (Weir
...
class ManagerLibraryDescriptor:
    ...
    necessarily a path Weir can see.
    """
...
class MediaManagerPort(Protocol):
    ...
    def library_truth(
        ...
        *,
        ...
        this one folder still yours" without leaking Weir's paths into the query.
        """
