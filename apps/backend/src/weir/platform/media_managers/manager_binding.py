...
from weir.platform.media_managers.manager_dialects import environment_connection_for_scope, port_for_kind
...
def _connection_from_row(settings: WeirSettings, row: MediaManagerConnectionRow) -> ManagerConnection | None:
...
def all_enabled_connections(session: Session, settings: WeirSettings) -> list[ManagerConnection]:
...
def describe_connections(session: Session, settings: WeirSettings) -> tuple[ManagerDescription, ...]:
