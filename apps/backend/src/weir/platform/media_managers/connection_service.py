...
from weir.platform.outbound_http import normalize_local_service_base_url
...
def rotate_webhook_secret(session: Session, settings: WeirSettings, row: MediaManagerConnectionRow) -> str:
...
def webhook_secret_matches(settings: WeirSettings, row: MediaManagerConnectionRow, presented: str | None) -> bool:
