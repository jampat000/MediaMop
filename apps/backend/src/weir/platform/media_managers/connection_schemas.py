...
class MediaManagerConnectionOut(BaseModel):
    ...
    name: str
    enabled: bool
    base_url: str = Field(description="Address Weir uses to reach this manager. Empty means not set.")
...
class MediaManagerWebhookSecretOut(BaseModel):
    """The generated secret, shown once. Weir keeps only the encrypted copy."""
...
class MediaManagerCapabilityOut(BaseModel):
    ...
    reports_import_queue: bool = Field(description="Whether Weir can ask this manager what it is currently importing.")
    ...
    reachable: bool = Field(description="Whether the manager answered when Weir asked what it manages.")
    library_roots: list[str] = Field(default_factory=list, description="Folders this manager reported that it manages.")
    summary: str = Field(description="One plain-language sentence about what Weir can do with this manager.")
    detail: str | None = Field(default=None, description="Why Weir could not ask, when it could not.")
