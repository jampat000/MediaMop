...
A media manager tells Weir "here is a file, do your thing with it". Radarr and
...
def _normalize_native(body: Mapping[str, Any]) -> MediaManagerImportEvent | None:
    """Weir's own shape, for a manager with no dialect of its own.
...
MEDIA_MANAGER_DIALECTS: dict[str, MediaManagerDialect] = {
    ...
    for dialect in (
        ...
        MediaManagerDialect("native", "Generic (Weir native payload)", _normalize_native),
    )
}
