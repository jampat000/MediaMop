...
# the Deluno repo). ``native`` publishes the same routes because that is what Weir's
...
def _unreachable(connection: ManagerConnection, exc: Exception, *, what: str) -> str:
    ...
    if isinstance(exc, MediaManagerRateLimitedError):
        ...
        when = f" It asked Weir to wait about {int(wait)}s." if wait else ""
        return (
            f"{connection.label} is rate limiting Weir, so it could not say {what}.{when} "
    ...
    if isinstance(exc, MediaManagerHttpError):
        ...
        if "HTTP 401" in detail or "HTTP 403" in detail:
            return (
                f"{connection.label} refused Weir's API key, so it could not say {what}. "
                "Check the key on the Media managers settings page and save it again."
            )
        return f"{connection.label} did not give Weir the answer it expected when asked {what} ({detail})."
    return (
        f"Weir could not reach {connection.label} to ask {what} ({exc}). "
...
class ArrV3ManagerPort:
    ...
    def capabilities(self) -> ManagerCapabilities:
        ...
        return ManagerCapabilities(
            ...
            summary=(
                f"Looks after {scope_word}. Weir can ask it what is downloading or importing, "
    ...
    def remove_queue_item(self, connection: ManagerConnection, row: Mapping[str, Any]) -> None:
        ...
        200 is success. The download client removes the data; Weir deletes nothing itself.
        """
...
# Weir does not recognise must not read as "finished".
...
class ExternalIntegrationManagerPort:
    """Deluno, and anything else speaking Weir's own payload shape.
    ...
    **both** media scopes, so it answers for a Movies library and a TV one without a
    ...
    def capabilities(self) -> ManagerCapabilities:
        return ManagerCapabilities(
            ...
            summary=(
                "Looks after Movies and TV episodes. Weir can ask it what is mid-import, but not which "
    ...
    def library_truth(self, connection: ManagerConnection, *, media_scope: MediaScope) -> ManagerLibraryTruth:
        return ManagerLibraryTruth(
            ...
            status="no_signal",
            detail=(
                f"{connection.label} tells Weir what it manages and what it is importing, but not which "
...
def _manifest_library_descriptor(library: Mapping[str, Any]) -> ManagerLibraryDescriptor:
    ...
    workflow is what Weir branches on, not whether the path happens to be there.
    """
...
def _external_queue_status(entry: Mapping[str, Any], *, connection: ManagerConnection) -> str:
    ...
    if value:
        logger.debug(
            "Media manager %s reported queue state %r, which Weir treats as still in progress.",
            connection.label,
            raw,
        )
    return "downloading"
...
def environment_connection_for_scope(settings: WeirSettings, media_scope: MediaScope) -> ManagerConnection | None:
