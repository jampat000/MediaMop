...
from weir.platform.arr_library.arr_connection_crypto import decrypt_arr_api_key
...
from weir.platform.media_managers.connection_model import (
    MediaManagerConnectionRow,
    MediaManagerSearchLaneRow,
)
from weir.platform.media_managers.connection_schemas import (
...
from weir.platform.media_managers.manager_binding import describe_connections
from weir.platform.media_managers.manager_http import (
    MediaManagerHttpClient,
    MediaManagerHttpError,
)
from weir.platform.media_managers.schedule_csv_validate import (
    normalize_hhmm,
...
def _probe(name: str, kind: str, base_url: str, api_key: str | None) -> tuple[bool, str]:
    ...
    try:
        ...
        if "HTTP 401" in detail or "HTTP 403" in detail:
            return False, f"Weir reached {name}, but the API key was refused. Check the key and save it again."
        return False, (
            f"Weir reached {name} but did not get the answer it expected. "
            "Check the address points at the app itself, not a page inside it."
        )
    except OSError:
        return False, (
            f"Weir could not reach {name} at {base_url}. "
            "Check the address is right, and that the app is running and reachable from this machine."
        )
    return True, f"Connected. Weir can reach {name}."
