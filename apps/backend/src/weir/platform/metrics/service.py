...
class RuntimeMetricsStore:
    ...
    def render_prometheus(self) -> str:
        ...
        lines = [
            ...
            f"weir_process_uptime_seconds {summary['uptime_seconds']:.3f}",
            "# HELP weir_http_requests_total Total HTTP requests handled by Weir.",
            "# TYPE weir_http_requests_total counter",
            f"weir_http_requests_total {summary['total_requests']}",
            "# HELP weir_http_average_response_ms Average HTTP response time in milliseconds.",
            "# TYPE weir_http_average_response_ms gauge",
            f"weir_http_average_response_ms {summary['average_response_ms']:.3f}",
            "# HELP weir_log_records_total Total log records by level.",
            "# TYPE weir_log_records_total counter",
        ]
        for level in ("DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"):
            lines.append(f'weir_log_records_total{{level="{level.lower()}"}} {self.log_counts[level]}')
        for bucket, count in status_counts.items():
            lines.append(f'weir_http_status_total{{status="{bucket}"}} {count}')
        for module, counts in summary["module_job_counts"].items():
            for event, count in counts.items():
                lines.append(f'weir_module_jobs_total{{module="{module}",event="{event}"}} {count}')
        for module, depth in summary["module_queue_depths"].items():
            lines.append(f'weir_module_queue_depth{{module="{module}"}} {depth}')
        savings = summary["module_savings_bytes"]
        if savings:
            lines.append("# HELP weir_module_savings_bytes_total Bytes saved by each module (Refiner remux).")
            lines.append("# TYPE weir_module_savings_bytes_total counter")
            for module, total in sorted(savings.items()):
                lines.append(f'weir_module_savings_bytes_total{{module="{module}"}} {total}')
        return "\n".join(lines) + "\n"
