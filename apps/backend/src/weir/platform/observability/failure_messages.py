...
from weir.platform.observability.diagnostics import sanitize_diagnostic_value
from weir.platform.observability.operator_messages import provider_label
...
def _why_for_kind(kind: FailureKind, *, provider: str | None) -> str:
    ...
    if kind == "credential":
        return f"Weir could not use the saved credentials{where}."
    if kind == "auth":
        return f"The service{where} rejected the credentials or permission level."
    if kind == "network":
        return f"Weir could not reach the service{where} over the network."
    if kind == "filesystem":
        return "Weir could not use a file or folder it needed."
    if kind == "validation":
...
def _next_action_for_kind(kind: FailureKind, *, provider: str | None, recoverable: bool) -> str | None:
    ...
    if kind == "filesystem":
        return "Check that the file or folder still exists and that Weir can read and write it."
    if kind == "validation":
