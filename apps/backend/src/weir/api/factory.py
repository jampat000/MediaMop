...
from weir import __version__
from weir.api.router import build_v1_router
from weir.core.config import WeirSettings
from weir.core.lifespan import lifespan
from weir.platform.health import health_router
from weir.platform.http.compressed_assets import CompressedStaticAssetsMiddleware
from weir.platform.http.head_mirrors_get import HeadMirrorsGetMiddleware
from weir.platform.http.request_context import RequestContextMiddleware
from weir.platform.http.security_headers import SecurityHeadersMiddleware
from weir.platform.http.trusted_proxy import TrustedProxySchemeMiddleware
from weir.platform.http.xrw_csrf_middleware import XRequestedWithCsrfMiddleware
from weir.platform.metrics.router import router as metrics_router
from weir.platform.readiness import readiness_router


def _web_dist_root() -> Path | None:
    raw = (os.environ.get("WEIR_WEB_DIST") or "").strip()
    if not raw:
        return None
...
def _mount_web_spa_if_configured(application: FastAPI) -> None:
    """Serve the Vite production bundle from disk when ``WEIR_WEB_DIST`` is set."""
    root = _web_dist_root()
    if root is None:
        return
