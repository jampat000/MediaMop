...
from weir.integrations.metadata.provider_port import LookupResult, MetadataProvider
from weir.integrations.metadata.tmdb_provider import DEFAULT_TMDB_BASE_URL, TmdbMetadataProvider
...
#: The providers Weir can talk to. One today; the port exists so the second does not
#: require rewriting the callers.
KNOWN_PROVIDERS: tuple[str, ...] = ("tmdb",)


def store_provider_key(settings: WeirSettings, plaintext: str) -> str:
...
def build_provider(session: Session, settings: WeirSettings) -> MetadataProvider | None:
...
def test_provider(session: Session, settings: WeirSettings) -> LookupResult:
