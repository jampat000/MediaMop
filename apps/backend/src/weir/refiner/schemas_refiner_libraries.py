...
class RefinerLibraryCreateIn(BaseModel):
    ...
    output_collision_policy: Literal["replace", "skip", "keep_both", "replace_if_larger", "replace_if_newer"] = Field(
        "replace",
        description=(
            "What to do when an output already exists at the same path. 'replace' is what Weir has "
    ...
    hardware_decode_mode: Literal["off", "auto", "device"] = Field(
        "off",
        description=(
            "Hardware decoding. 'off' is what Weir has always done. A choice that cannot work falls back "
    ...
    file_detection_interval_seconds: int = Field(
        ...
        le=3600,
        description=(
            "How long this file's size must stay unchanged before Weir treats it as finished being "
    ...
    max_attempts: int = Field(
        3, ge=1, le=20, description="How many times Weir tries a file on its own before stopping."
    )
    ...
    failure_policy: Literal["pass_through", "hold", "reject"] = Field(
        ...
        description=(
            ...
            "folder unchanged, so a file Weir cannot process still reaches your media manager. "
            "'hold' keeps it with Weir until someone acts. 'reject' tells the media manager the release "
    ...
    schedule_grid: str = Field(
        ...
        description=(
            ...
            "Work already running finishes when a window closes; the window stops Weir starting anything new."
        ),
    )
    file_system_events_enabled: bool = Field(
        ...
        description=(
            ...
            "runs regardless, so switching this off makes Weir slower to notice a file, never blind to it."
        ),
    )
