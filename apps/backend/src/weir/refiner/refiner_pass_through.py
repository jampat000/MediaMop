...
Weir sits in the middle of a pipeline it does not own. A media manager downloads a release
into a completed folder, Weir takes it, and writes a result into an output folder the manager
imports. When processing failed and retries ran out, the file used to stay in Weir's hands for
...
from weir.platform.file_lifecycle.mutations import safe_copy_to_final
from weir.platform.jobs.worker_failures import AlreadyRecordedFailure
...
from weir.refiner.refiner_output_collision import decide_output_collision
...
def _make_byte_integrity_check(source: Path) -> Callable[[Path], None]:
    ...
    def check(staged: Path) -> None:
        ...
        if after != before:
            msg = "The source changed while Weir was copying it, so the copy was not delivered."
...
def apply_failure_policy(
    ...
    *,
    ...
    once Weir has genuinely given up on processing the file.
    """
    ...
    if bad_release and reject_bad_release(
        ...
        library=library,
        relative_path=relative_path,
        reason=row.status_reason if row is not None else "Weir could not read this file's contents.",
        origin=origin,
    ):
        if row is not None:
            row.status_reason = (
                f"{row.status_reason} Weir is telling your media manager this release is bad so it can find "
                "a different one."
            ).strip()[:10000]
        return FAILURE_POLICY_REJECT

    # Any other failure that reaches this point is not evidence the release is bad: a path Weir cannot
    ...
    if row is not None:
        ...
        row.status_reason = (
            f"{row.status_reason} Weir could not process this file, so it is handing the original "
...
def deliver_unchanged(settings: DeliverySettings, *, relative_path: str) -> DeliveryResult:
    ...
    if decision.wrote:
        ...
        sentence = (
            "Weir could not process this file, so it handed the original back unchanged to "
            f"{decision.destination}. Your media manager can import it as normal."
        )
    else:
        reason = decision.reason
        sentence = (
            "Weir could not process this file and would have handed the original back unchanged, "
