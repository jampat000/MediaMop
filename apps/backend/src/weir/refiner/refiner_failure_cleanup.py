...
from weir.platform.media_managers.manager_binding import collect_queue_signals
from weir.platform.media_managers.manager_port import ManagerQueueSignal
...
from weir.refiner.manager_queue_signals import attributed_rows_for_file, report_for_signals
...
def run_refiner_failure_cleanup_sweep_for_scope(
    *,
    ...
    if library is not None:
        work_raw, _ = effective_library_work_folder(library=library, weir_home=settings.weir_home)
    ...
    for job, rel_norm, legacy_dry_run in failed_rows:
        ...
        if queue_unreachable:
            detail[f"{'tv' if ms == 'tv' else 'movie'}_failure_cleanup_skip_reason"] = (
                f"Weir could not check whether anything is still importing, so nothing was removed. {queue_unreachable}"
            )
            continue
