...
this candidate. ``no_upstream_signal`` means Weir could not get that answer at all,
...
from weir.refiner.domain import FileAnchorCandidate
from weir.refiner.manager_queue_signals import (
...
def evaluate_refiner_candidate_gate_from_manager_signals(
    *,
    ...
    if not report.has_any_signal:
        return _outcome(
            "no_upstream_signal",
            "No connected media manager could say what it is importing, so Weir has no upstream check "
    ...
    if blocked_by is not None:
        return _outcome(
            "wait_upstream",
            f"{blocked_by} is still importing this file, so Weir treats this candidate as held upstream.",
        )
