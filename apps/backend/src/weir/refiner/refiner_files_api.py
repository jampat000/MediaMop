...
from weir.refiner.direct_play.service import direct_play_for_row, selected_device_profiles
from weir.refiner.jobs_ops import move_refiner_job_to_top
from weir.refiner.refiner_file_log_service import logs_for_file, render_log_text
from weir.refiner.refiner_file_state_model import RefinerFileRow
from weir.refiner.refiner_file_state_service import forget_file, list_files, status_counts
from weir.refiner.refiner_file_story import narrate_pass
from weir.refiner.refiner_job_queue_lookup import pending_remux_job_for_relative_path
from weir.refiner.refiner_library_model import RefinerLibraryRow
from weir.refiner.refiner_live_progress import live_progress_by_path
from weir.refiner.refiner_operator_settings_service import ensure_refiner_operator_settings_row
from weir.refiner.refiner_requeue_service import requeue_file, requeue_files
...
def get_refiner_files(
    ...
    files = [
        RefinerFileOut(
            ...
            output_collision_policy=row.output_collision_policy,
            output_collision_action=row.output_collision_action,
...
def delete_refiner_file(
    ...
    """Forget a file. Removes Weir's record of it, never the file on disk."""

    _verify_csrf(request, settings, body.csrf_token)
    row = db.get(RefinerFileRow, file_id)
    if row is None:
        raise HTTPException(status_code=http_status.HTTP_404_NOT_FOUND, detail="Weir has no record of that file.")
    forget_file(db, row)
    db.commit()
...
def get_refiner_file_log(
    ...
    """Everything Weir retained about what it did to this file, newest first."""
...
def download_refiner_file_log(
    ...
    return PlainTextResponse(
        ...
        headers={"Content-Disposition": f'attachment; filename="weir-{safe}.log.txt"'},
    )
