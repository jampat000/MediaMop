...
def disabled_watch_reports(session: Session) -> list[WatcherReport]:
    ...
    return [
        WatcherReport(
            ...
            detail=(
                f"Filesystem events are switched off for {row.name}, so Weir finds new files on the scan interval."
            ),
        )
        for row in rows
...
def _debounce_seconds(settings: WeirSettings) -> float:
    ...
    for library in libraries:
        ...
        try:
            if not folder.is_dir():
                raise OSError(f"{folder} is not a folder Weir can see")
            ...
            reports.append(
                WatcherReport(
                    ...
                    detail=(
                        f"Weir could not watch {folder} for changes, so it is finding new files on the "
            ...
            for row in libraries:
                record_watcher_state(
                    WatcherReport(
                        ...
                        status=WatcherStatus.POLLING_FALLBACK,
                        detail=(
                            "The filesystem watcher is not available in this environment, so Weir finds "
