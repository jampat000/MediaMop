...
from weir.platform.file_lifecycle.guardrails import bytes_to_mb, check_minimum_free_disk_space
from weir.platform.file_lifecycle.mutations import (
    safe_copy_to_final,
    safe_finalize_file,
    try_hardlink_to_final,
)
from weir.platform.metrics.service import record_module_savings
from weir.refiner.file_remux_pass.paths import resolve_media_file_under_refiner_root
...
from weir.refiner.refiner_file_settling import acquire_source_read_guard
...
from weir.refiner.refiner_output_collision import (
...
from weir.refiner.refiner_remux_mux import (
...
from weir.refiner.refiner_runner_units import video_dimensions_from_streams
from weir.refiner.refiner_sidecar_migration import (
    apply_original_timestamps,
    migrate_sidecars,
    parse_sidecar_patterns,
)
from weir.refiner.refiner_tv_output_cleanup import (
    maybe_run_tv_output_season_folder_cleanup_after_remux,
)
from weir.refiner.refiner_tv_season_folder_cleanup import (
...
def _migrate_sidecars_before_cleanup(
    *,
    ...
    copy of a file Weir was asked to keep (#344).
    """
...
def _handle_refiner_cleanup_after_success(
    *,
    ...
    # a file Weir was asked to keep (#344).
    if out.get("sidecar_migration_blocked"):
        ...
        out["source_folder_skip_reason"] = out.get("sidecar_migration_blocked_reason") or (
            "Weir did not remove the source folder because a file set to travel with the video could not be copied."
        )
...
def _run_refiner_file_remux_pass(
    *,
    ...
    try:
        ...
        return _fail_before(
            relative_media_path=relative_media_path,
            reason=f"Weir could not inspect this file under the saved watched folder: {exc}",
    ...
    if not exists:
        return _fail_before(
            relative_media_path=relative_media_path,
            reason=(
                "Weir could not find this file under the saved watched folder. "
    ...
    try:
        ...
        return _fail_before(
            relative_media_path=relative_media_path,
            reason=f"Weir could not read this file's contents, so the file itself looks damaged: {exc}",
            inspected_source_path=inspected,
    ...
    if os.name != "nt":
        try:
            ...
            validate_media_integrity(src, weir_home=settings.weir_home)
            _assert_source_unchanged(src, expected_source_fingerprint)
    ...
    if not pass_through_unchanged:
        _, ffmpeg_bin = resolve_ffprobe_ffmpeg(weir_home=settings.weir_home)
            ...
            progress_reporter(
                {
                    ...
                    "message": (
                        ...
                        if pass_through_unchanged
                        else "Weir is copying the unchanged file to the output folder."
                    ),
                }
            )
...
def run_refiner_file_remux_pass(
    *,
    ...
    if guard is None:
        return _source_not_ready(
            relative_media_path=relative_media_path,
            reason=problem or "Weir is waiting until no other program is writing this file.",
    ...
    with guard:
        try:
            ...
            return _source_not_ready(
                relative_media_path=relative_media_path,
                reason=f"Weir could not read this file safely ({exc}), so it will wait.",
                inspected_source_path=str(source.resolve()),
            )
        return _run_refiner_file_remux_pass(
            settings=settings,
