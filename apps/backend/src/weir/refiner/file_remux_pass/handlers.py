...
from weir.platform.activity.provenance import job_provenance
from weir.platform.media_managers.completion_callback import report_handoff_completion
from weir.refiner.file_remux_pass.run import run_refiner_file_remux_pass
...
from weir.refiner.refiner_failure_classes import classify_failure
from weir.refiner.refiner_file_log_service import record_file_log
...
from weir.refiner.refiner_library_service import resolve_library, rules_config_for
...
from weir.refiner.refiner_remux_rules_settings_service import load_refiner_remux_rules_config
from weir.refiner.refiner_requeue_service import record_failure
        ...
        with session_factory() as session, session.begin():
            ...
            if result.get("rejection_kind"):
                ...
                if reject_bad_release(
                    ...
                    cleanup_detail = (
                        "Weir is telling your media manager this release is bad so it can find a different one, "
            ...
            if result.get("retryable_wait") is True:
                reason = " ".join(
                    str(result.get("reason") or "Weir is waiting until no other program is writing this file.").split()
                )[:1200]
                row = mark_file_status(
                    session,
                    library_id=library.id,
            ...
            if result.get("ok") is False:
                ...
                follow_up = apply_failure_policy(
                    session,
                    library=library,
                    relative_path=relative_path.strip(),
                ...
                result.update(
                    {
                        "pass_through_queued": follow_up == FAILURE_POLICY_PASS_THROUGH,
                        "reject_queued": follow_up == FAILURE_POLICY_REJECT,
    ...
    *,
    ...
    if not isinstance(watched_root, str) or not isinstance(inspected_source, str):
        ...
        result["rejected_cleanup_detail"] = (
            "Weir did not delete the rejected file because its verified watched-folder path was unavailable."
        )
    else:
            ...
            if path_err is not None:
                ...
                if result.get("rejection_kind") and library is not None:
                    ...
                    if rejected_action == "delete_file":
                        ...
                        result["rejected_cleanup_detail"] = (
                            "This library is set to delete rejected files. Weir recorded the rejection and will now remove only this file."
                        )
                    else:
                        result["rejected_cleanup_status"] = "left_in_place"
                        result["rejected_cleanup_detail"] = (
                            "Weir left the rejected file in place because this library's cleanup action is Leave in place."
                        )
