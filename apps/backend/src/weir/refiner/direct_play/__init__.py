...
already decided what the file should be when it grabbed it; Weir does what it is configured to
...
replace it without a release by putting a copy at ``WEIR_HOME/direct-play-devices.json``.
...
def load_device_profiles(weir_home: str | None) -> tuple[DeviceProfile, ...]:
    """The operator's copy when there is a readable one, otherwise the shipped list."""

    if weir_home:
        override = Path(weir_home) / OVERRIDE_FILE_NAME
        if override.is_file():
            try:
...
def evaluate(profile: DeviceProfile, facts: MediaFacts) -> DirectPlayVerdict:
    ...
    if unknown:
        return DirectPlayVerdict(profile.id, profile.name, "unknown", ("Weir has not measured this file yet",))
