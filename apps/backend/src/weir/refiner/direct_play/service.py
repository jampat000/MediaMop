...
from weir.refiner.schemas_refiner_files import DirectPlayOut
...
def save_selected_device_ids(session: Session, settings: WeirSettings, selected: list[str]) -> list[str]:
    """Keep only ids the current device list knows, in the list's own order."""

    known = [profile.id for profile in load_device_profiles(settings.weir_home)]
    wanted = set(selected)
...
def selected_device_profiles(session: Session, settings: WeirSettings) -> tuple[DeviceProfile, ...]:
    chosen = set(selected_device_ids(session))
    if not chosen:
        return ()
    return tuple(p for p in load_device_profiles(settings.weir_home) if p.id in chosen)


def device_list_is_customised(settings: WeirSettings) -> bool:
    return bool(settings.weir_home) and (Path(settings.weir_home) / OVERRIDE_FILE_NAME).is_file()
