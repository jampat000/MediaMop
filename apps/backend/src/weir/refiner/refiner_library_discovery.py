...
The operator typing a watched folder into Weir is re-entering information the
...
destructive surprise. Drift is surfaced with the manager's value and Weir's own value
side by side, and the operator decides.

**A path on the manager's host is not automatically a path Weir can see.** The check
...
from weir.refiner.refiner_library_service import list_libraries
...
@dataclass(frozen=True, slots=True)
class LibraryDrift:
    """A difference between what the manager says and what Weir has saved."""
...
def _looks_absolute(raw: str) -> bool:
    ...
    checked here, and whether Weir can actually see it is the next question.
    """
...
def local_path_problem(root_path: str | None) -> str | None:
    ...
    if not raw:
        return (
            "The manager did not say where this library lives, so Weir has no folder to watch. "
            "Set the watched folder yourself after importing."
        )
    if not _looks_absolute(raw):
        return f"The manager reported {raw!r}, which is not an absolute path Weir can resolve."
    if not Path(raw).expanduser().is_dir():
        return (
            ...
            "Weir — both hosts have to see the same folder at the same path. Mount it there, or set "
...
def _descriptors_for(
    ...
    if not resolved:
        raise RefinerDiscoveryError(
            f"{connection_row.name} has no saved address and API key, so Weir cannot ask it anything."
        )
    port = port_for_kind(connection_row.kind)
    if port is None:
        raise RefinerDiscoveryError(f"Weir does not know how to talk to a {connection_row.kind} connection.")
...
def discoverable_libraries(
    ...
    """What this manager reports, marked up with what Weir already has."""
...
def import_libraries(
    ...
    *,
    ...
    for key in wanted:
        ...
        # Weir cannot see: a library pointed at a folder that is not there would
        ...
        # A manager that processes before importing tells Weir where it expects the
        ...
        # reason: the manager's path is not necessarily one Weir can see.
        usable_output = (
...
def resync_drift(
    ...
    """Differences between the manager and Weir. Reported only, never applied."""
        ...
        if descriptor is None:
            drift.append(
                LibraryDrift(
                    ...
                    manager_value=None,
                    weir_value=row.watched_folder or None,
                    detail=(
                        f"{connection_row.name} no longer reports this library. Weir has left it exactly as "
        ...
        if manager_root and saved and _comparable(manager_root) != _comparable(saved):
            drift.append(
                LibraryDrift(
                    ...
                    weir_value=saved,
                    detail=(
                        f"{connection_row.name} now says this library lives at {manager_root!r}, but Weir is "
    ...
    for key, descriptor in descriptors.items():
        ...
        drift.append(
            LibraryDrift(
                ...
                weir_value=None,
                detail=(
                    f"{connection_row.name} reports a library Weir has not imported. Import it if you want "
