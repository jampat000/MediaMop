...
from weir.platform.file_lifecycle.mutations import FileLifecycleError, safe_copy_to_final
...
def apply_original_timestamps(*, source_media: Path, output_media: Path) -> str | None:
    ...
    try:
        ...
        return f"Weir could not read the original file's timestamps ({exc})."
    try:
        import os

        os.utime(output_media, (stat.st_atime, stat.st_mtime))
    except OSError as exc:
        return f"Weir could not apply the original timestamps to the output ({exc})."
    return None
