...
from weir.platform.activity.provenance import SCAN_TRIGGER_TO_TRIGGER
from weir.platform.media_managers.manager_port import MediaScope
...
from weir.refiner.refiner_requeue_service import requeue_file
from weir.refiner.refiner_runner_units import (
        ...
        with session_factory() as session, session.begin():
            for file_path in files:
                ...
                if library is not None:
                    ...
                    if previous is not None and previous.size_bytes == observed_size:
                        ...
                        # A rejected file waits for its manager (or Weir) to remove it; picking it
                        ...
                        if media_scope == "movie":
                            ...
                            if cleanup_ok:
                                ...
                                for sibling_row in sibling_rows:
                                    ...
                                    sibling_row.status_reason = "Weir removed this file with its release folder after the validated movie output completed. No separate output was created for this extra file."
                                ...
                                record_file_state(
                                    ...
                                    verdict=FileStateVerdict(
                                        ...
                                        "The output was already complete. The temporary lock cleared, so Weir finished removing the source release folder.",
                                    ),
                                    size_bytes=observed_size,
                                    size_changed_at=settling.size_changed_at,
                                )
                                ...
                                record_file_state(
                                    ...
                                    verdict=FileStateVerdict(
                                        ...
                                        f"The output is complete, but source cleanup is waiting: {retry_reason} Weir will try again automatically.",
                                    ),
                                    size_bytes=observed_size,
                                    size_changed_at=settling.size_changed_at,
                                )
                    ...
                    if rejection is not None:
                        ...
                        if cleanup_action == "delete_file":
                            ...
                            reason = (
                                ...
                                "Weir will record this decision before removing only this file."
                            )
                        record_file_state(
                            session,
                            library=library,
                ...
                # A file Weir has not processed before costs the "undetermined"
            ...
            if queued:
                ...
                held_by = (
                    ...
                    if len(upstream_block_reasons) == 1
                        ...
                        "or imported, so Weir left them alone for now."
                    )
                )
                summary["user_message"] = held_by
                summary["waiting_message"] = "Weir will check again on the next scheduled scan."
                ...
                summary["user_message"] = "Weir found matching media files, but they were already waiting for Refiner."
            elif seen:
                summary["scan_result_label"] = "nothing_new"
                summary["user_message"] = "Weir found media files, but there was nothing new to queue."
            else:
                summary["scan_result_label"] = "no_media_found"
                summary["user_message"] = "Weir did not find any media files in this watched folder."
            ...
            if ignored:
                ...
                summary["user_message"] = (
                    ...
                    f"{'was' if ignored == 1 else 'were'} ignored because Weir does not process "
