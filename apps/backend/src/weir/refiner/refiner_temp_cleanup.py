...
from weir.refiner.refiner_library_service import list_libraries, seeded_library_for_scope
from weir.refiner.refiner_operator_settings_service import (
...
def _resolved_movie_and_tv_work_roots(*, session: Session, settings: WeirSettings) -> tuple[Path, Path]:
    ...
    movie_work = (
        effective_library_work_folder(library=movie, weir_home=settings.weir_home)[0]
        if movie is not None
        else resolved_default_refiner_work_folder(weir_home=settings.weir_home)
    )
    tv_work = (
        effective_library_work_folder(library=tv, weir_home=settings.weir_home)[0]
        if tv is not None
        else resolved_default_refiner_tv_work_folder(weir_home=settings.weir_home)
    )
    return Path(movie_work).expanduser().resolve(), Path(tv_work).expanduser().resolve()


def resolved_library_work_roots(*, session: Session, settings: WeirSettings) -> dict[int, Path]:
    """Every library's work root, so a library with a custom one is not left unswept."""

    out: dict[int, Path] = {}
    for library in list_libraries(session):
        work, _ = effective_library_work_folder(library=library, weir_home=settings.weir_home)
    ...
    *,
    ...
    if operator.keep_failed_work_files:
        ...
        out["temp_cleanup_skipped_reason"] = (
            ...
            "failed run can be inspected. Turn it off to let Weir reclaim them again."
        )
        return out
