...
Binary resolution: ``<WEIR_HOME>/bin/ffmpeg/{ffprobe,ffmpeg}[.exe]`` first, then ``PATH``.
"""
...
from weir.refiner.refiner_metadata_rules import metadata_argv_flags
from weir.refiner.refiner_remux_rules import RemuxPlan
...
def resolve_ffprobe_ffmpeg(*, weir_home: str) -> tuple[str, str]:
    """Prefer bundled tools under ``weir_home``, then host ``PATH``."""

    home = Path(weir_home).expanduser().resolve()
    candidate_dirs: list[Path] = []
    raw_env_dir = (os.environ.get("WEIR_FFMPEG_DIR") or "").strip()
    if raw_env_dir:
...
def ffprobe_json(
    ...
    *,
    ...
    ffprobe, _ = resolve_ffprobe_ffmpeg(weir_home=weir_home)
    try:
...
def validate_remux_output(
    ...
    *,
    ...
    data = ffprobe_json(path, weir_home=weir_home)
...
def validate_media_integrity(path: Path, *, weir_home: str) -> None:
    ...
    _, ffmpeg_bin = resolve_ffprobe_ffmpeg(weir_home=weir_home)
    argv = [
        ffmpeg_bin,
        "-hide_banner",
        "-v",
