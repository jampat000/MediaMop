...
from weir.refiner.refiner_library_crud import (
    RefinerLibraryError,
...
from weir.refiner.refiner_library_model import RefinerLibraryRow, RefinerRuleSetRow
from weir.refiner.refiner_library_service import list_libraries, manager_connection_ids_for
from weir.refiner.refiner_pass_through import FAILURE_POLICY_REJECT, normalize_failure_policy
from weir.refiner.refiner_reject import reject_support
from weir.refiner.schemas_refiner_libraries import (
...
def _library_out(db, row: RefinerLibraryRow) -> RefinerLibraryOut:
    ...
    return RefinerLibraryOut(
        ...
        rejected_file_action=row.rejected_file_action or "leave",
        min_file_age_seconds=row.min_file_age_seconds,
        created_after=row.created_after,
        created_before=row.created_before,
        ...
        output_collision_policy=row.output_collision_policy or "replace",
        hardware_decode_mode=row.hardware_decode_mode or "off",
        hardware_device=row.hardware_device or "",
        hardware_disabled_vendors_csv=row.hardware_disabled_vendors_csv or "",
        ffmpeg_strictness=row.ffmpeg_strictness or "normal",
...
def _refuse_unsupported_reject(db, settings: WeirSettings, row: RefinerLibraryRow) -> None:
...
def _rule_set_out(db, row: RefinerRuleSetRow) -> RefinerRuleSetOut:
    return RefinerRuleSetOut(
        ...
        keep_original_language=bool(row.keep_original_language),
        original_language_additional_csv=row.original_language_additional_csv or "",
        original_language_keep_only_first=bool(row.original_language_keep_only_first),
        original_language_first_if_none=bool(row.original_language_first_if_none),
        original_language_treat_empty_as_original=bool(row.original_language_treat_empty_as_original),
...
def get_refiner_discoverable_libraries(
    ...
    """What this manager says it looks after, and whether Weir already has it."""
...
def get_refiner_library_drift(
    ...
    """Differences between the manager and Weir. Reported only — nothing is applied."""
