...
def _planned(detail: dict[str, Any], ok: bool) -> StoryStep | None:
    ...
    sentence = f"Weir planned to {', '.join(changes)}." if changes else "Weir planned the output tracks."
    if audio_after:
...
def _verified(detail: dict[str, Any]) -> StoryStep | None:
    ...
    if check == "passed":
        return StoryStep("Checked", "Weir checked the finished file was complete before handing it on.", "good")
    if check == "failed":
        return StoryStep("Checked", note or "The finished file did not pass Weir's completeness check.", "bad")
    return None
...
def _failed(detail: dict[str, Any]) -> StoryStep | None:
    ...
    return StoryStep(
        "Could not finish",
        reason or "Weir could not process this file, and the record does not say why.",
        "bad",
    )
