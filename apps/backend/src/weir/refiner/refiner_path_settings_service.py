...
from weir.refiner.refiner_library_service import resolve_library, seeded_library_for_scope

RefinerMediaScope = Literal["movie", "tv"]

_REFINER_LEGACY_WINDOWS_MOVIE_WORK = Path(r"C:\ProgramData\Media\refiner-movie-work")
_REFINER_LEGACY_WINDOWS_TV_WORK = Path(r"C:\ProgramData\Weir\refiner-tv-work")


def resolved_default_refiner_work_folder(*, weir_home: str) -> str:
    """Default Movies work/temp directory.

    Defaults are under ``WEIR_HOME`` so packaged Windows installs use
    ``C:\\ProgramData\\Weir`` while Docker/dev installs follow their configured
    runtime root.
    """

    return str(Path(weir_home).expanduser().resolve() / "refiner" / "refiner-movie-work")


def resolved_default_refiner_tv_work_folder(*, weir_home: str) -> str:
    ...
    return str(Path(weir_home).expanduser().resolve() / "refiner" / "refiner-tv-work")
...
def resolve_refiner_path_runtime_for_library(
    ...
    work_str, work_is_default = effective_library_work_folder(library=library, weir_home=settings.weir_home)
...
def effective_library_work_folder(*, library: RefinerLibraryRow, weir_home: str) -> tuple[str, bool]:
    ...
    if not raw or _is_legacy_refiner_default_work_folder(raw, media_scope=scope):  # type: ignore[arg-type]
        default = (
            resolved_default_refiner_tv_work_folder(weir_home=weir_home)
            if scope == "tv"
            else resolved_default_refiner_work_folder(weir_home=weir_home)
        )
        return default, True
    return raw, False
