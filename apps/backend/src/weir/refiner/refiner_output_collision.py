...
def _next_free_path(final: Path, *, limit: int = 999) -> Path:
    ...
    Weir cannot read, and a random suffix would produce a different name every run
    ...
    *,
    ...
    if not final.exists():
        return CollisionDecision(
            ...
            reason="No file existed at the output path, so Weir wrote it there.",
        )
    ...
    if chosen == "keep_both":
        ...
        return CollisionDecision(
            ...
            replaced_existing=False,
            reason=(
                f"An output already exists at {final.name}, so Weir wrote this one alongside it as {destination.name}."
            ),
        )
    ...
    if chosen == "replace_if_larger":
        ...
        if existing_size is None or new_size is None:
            return CollisionDecision(
                ...
                replaced_existing=False,
                reason=(
                    f"Weir could not compare the new output with the existing {final.name}, so it kept the "
        ...
        if new_size > existing_size:
            return CollisionDecision(
                ...
                reason=(
                    ...
                    f"({new_size} bytes against {existing_size}), so Weir replaced it."
                ),
            )
        return CollisionDecision(
            policy=chosen,
            ...
            reason=(
                ...
                f"({new_size} bytes against {existing_size}), so Weir kept the existing one."
            ),
        )
    ...
    if chosen == "replace_if_newer":
        ...
        if existing_mtime is None or source_mtime is None:
            return CollisionDecision(
                ...
                replaced_existing=False,
                reason=(
                    f"Weir could not compare timestamps with the existing {final.name}, so it kept the "
        ...
        if source_mtime > existing_mtime:
            return CollisionDecision(
                ...
                destination=final,
                replaced_existing=True,
                reason=(f"The source is newer than the existing output at {final.name}, so Weir replaced it."),
            )
        return CollisionDecision(
            policy=chosen,
            action="skip",
            ...
            replaced_existing=False,
            reason=(
                f"The source is not newer than the existing output at {final.name}, so Weir kept the existing one."
            ),
        )
