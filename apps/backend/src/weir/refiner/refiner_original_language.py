...
from weir.integrations.metadata.provider_port import LookupResult
from weir.refiner.refiner_remux_rules import normalize_lang
...
def canonical_language(raw: str | None) -> str:
    ...
    A code Weir does not know is returned as it is rather than dropped: an unlisted
...
def select_original_language_tracks(
    *,
    ...
    if rules.first_if_none and tracks:
        ...
        return OriginalLanguageOutcome(
            note=(
                f"No audio track matched the original language ({original}), so Weir fell back to the "
