...
:mod:`weir.refiner.domain` decides *whether* a row blocks a file. It has no
...
from weir.platform.media_managers.manager_port import ManagerQueueSignal, MediaScope
...
from weir.refiner.queue_adapter import map_queue_row_to_refiner_view, queue_dialect_for_scope
...
def attributed_queue_rows(
    ...
    *,
    ...
    :func:`weir.platform.media_managers.manager_binding.collect_queue_signals`
...
def upstream_block_reason(
    ...
    *,
    ...
    if label is None:
        return None
    return f"{label} is still importing this file, so Weir left it alone for now."
...
class QueueSignalReport:
    ...
    def note(self) -> str | None:
        ...
        return f"Weir could not get an import check from {names}, so it did not treat that as 'nothing is importing'."
...
def no_manager_configured_note(media_scope: str) -> str:
    ...
    return (
        f"No media manager is connected for {scope_word}, so Weir had no import check to make. "
