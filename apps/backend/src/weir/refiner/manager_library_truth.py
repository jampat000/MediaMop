...
from weir.platform.media_managers.manager_port import ManagerLibraryTruth
    ...
    *,
    ...
    if not answers:
        return LibraryTruthVerdict(
            check="skipped",
            note=(
                f"No media manager is connected for {scope_word}, so Weir could not check whether this "
    ...
    if silent:
        ...
        note = (
            f"Weir could not confirm with {names} whether this folder still holds library files, "
    ...
    if hits:
        return LibraryTruthVerdict(
            check="failed",
            note=(
                f"{', '.join(holders)} still keeps at least one library file inside this folder, so Weir "
    ...
    return LibraryTruthVerdict(
        check="passed",
        note=(
            f"{names} reported no library files inside this folder, so Weir treated it as safe to remove "
