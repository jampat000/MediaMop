...
def cleanup_refiner_partial_output_files(session: Session, settings: WeirSettings) -> int:
    """Remove hidden ``*.partial`` files left by interrupted atomic output writes.

    Weir writes final outputs through hidden partial files and an atomic replace. On process
    ...
    roots.add(Path(settings.weir_home).expanduser() / "refiner-output")
