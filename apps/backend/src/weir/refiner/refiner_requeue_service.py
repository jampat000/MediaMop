...
from weir.refiner.refiner_failure_classes import (
    RefinerFailureClass,
    ...
    *,
    ...
    if attempts_so_far >= max_attempts:
        return RetryDecision(
            ...
            next_retry_at=None,
            reason=(
                f"Weir tried this file {attempts_so_far} times and stopped, because the {library.name} "
    ...
    return RetryDecision(
        ...
        next_retry_at=when,
        reason=(
            f"This failed and Weir will try again in about {delay // 60 or 1} minute(s) "
...
def _not_retryable_reason(failure_class: str, library: RefinerLibraryRow) -> str:
    ...
    return (
        "Weir could not work out why this failed, so it is not retrying automatically. You can start it again by hand."
    )
    ...
    *,
    ...
    if consecutive_failures >= REFINER_QUARANTINE_AFTER_FAILURES:
        decision = RetryDecision(
            ...
            quarantined=True,
            reason=(
                f"Weir held this file after {consecutive_failures} repeated {value} failures. "
