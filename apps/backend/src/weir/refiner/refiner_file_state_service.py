...
2. **Out of schedule** — Weir is paused, or the library is on but not right now.
    ...
    *,
    ...
    if not library.enabled:
        return FileStateVerdict(
            ...
            f"The {library.name} library is switched off, so Weir is leaving its files alone.",
        )

    if paused_reason:
        # A paused instance is out of schedule in the only sense that matters to someone
        # looking at the screen: Weir is deliberately not working on this right now,
    ...
    if library.schedule_enabled and not in_schedule_window:
        return FileStateVerdict(
            ...
            (
                ...
                "Weir will pick this up when the window opens."
            ),
    ...
    if size_is_settling:
        return FileStateVerdict(
            ...
            settling_reason
            or "This file is still being written to, so Weir is waiting for it to finish before touching it.",
    ...
    if hold_seconds and file_age_seconds is not None and file_age_seconds < hold_seconds:
        ...
        return FileStateVerdict(
            RefinerFileStatus.ON_HOLD,
            (
                f"This file changed too recently. Weir waits {hold_seconds}s after the last change before "
    ...
    if access_problem:
        ...
        # scan re-checks it. A countdown here would be a number Weir invented.
    ...
    if blocked_by_connection:
        return FileStateVerdict(
            ...
            f"{blocked_by_connection} is still importing this file, so Weir left it alone for now.",
...
def mark_file_status(
    ...
    *,
    ...
    """Move a file Weir has already seen into a new state (processing, processed, failed)."""
