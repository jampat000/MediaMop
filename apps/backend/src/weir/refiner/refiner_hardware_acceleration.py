...
**Detection is read-only and honest.** Weir reports what ffmpeg says it was built
...
#: what Weir has always effectively used by not passing the flag.
...
def detect_acceleration(ffmpeg_bin: str) -> AccelerationReport:
    ...
    try:
        ...
        return AccelerationReport(
            detected=False,
            detail=f"Weir could not ask ffmpeg which acceleration methods it supports ({exc}).",
        )
    ...
    if completed.returncode != 0:
        return AccelerationReport(
            ...
            detail=(
                ...
                f"(exit code {completed.returncode}). Weir will use software decoding."
            ),
        )
...
class AccelerationDecision:
    """What Weir will actually ask ffmpeg for, and why."""
...
def decide_acceleration(
    *,
    ...
    if settings.mode == "device":
        ...
        if not wanted:
            return AccelerationDecision(
                ...
                fell_back_to_software=True,
                reason=(
                    "Weir fell back to software decoding because this library is set to use a named "
        ...
        if wanted not in report.available_methods:
            return AccelerationDecision(
                ...
                fell_back_to_software=True,
                reason=(
                    f"Weir fell back to software decoding because the configured device '{wanted}' is not "
        ...
        if not _allowed(wanted, settings.disabled_vendors):
            return AccelerationDecision(
                ...
                fell_back_to_software=True,
                reason=(
                    f"Weir fell back to software decoding because '{wanted}' belongs to a vendor this "
    ...
    return AccelerationDecision(
        ...
        fell_back_to_software=True,
        reason=(
            "Weir fell back to software decoding because every acceleration method this ffmpeg build "
