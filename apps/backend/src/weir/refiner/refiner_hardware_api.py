...
from weir.refiner.refiner_remux_mux import resolve_ffprobe_ffmpeg
from weir.refiner.schemas_refiner_hardware import RefinerHardwareOut
...
def get_refiner_hardware(_user: UserPublicDep, settings: SettingsDep) -> RefinerHardwareOut:
    ...
    try:
        ...
        return RefinerHardwareOut(
            ...
            detail=f"Weir could not find ffmpeg, so it cannot report acceleration methods. {exc}",
        )
