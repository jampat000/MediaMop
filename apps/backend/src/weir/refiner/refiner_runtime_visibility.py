"""Map :class:`~weir.core.config.WeirSettings` to a bounded Refiner-only runtime snapshot."""

from __future__ import annotations

from weir.core.config import WeirSettings
from weir.refiner.refiner_remux_rules import refiner_media_extensions_sorted
...
_SQLITE_THROUGHPUT_NOTE = (
    "Weir stores durable jobs on SQLite. Several Refiner workers can each claim a different "
...
_WORK_TEMP_STALE_SWEEP_PERIODIC_NOTE = (
    ...
    "WEIR_REFINER_WORK_TEMP_STALE_SWEEP_MOVIE_SCHEDULE_INTERVAL_SECONDS and "
    "WEIR_REFINER_WORK_TEMP_STALE_SWEEP_TV_SCHEDULE_ENABLED / "
    "WEIR_REFINER_WORK_TEMP_STALE_SWEEP_TV_SCHEDULE_INTERVAL_SECONDS in apps/backend/.env (Refiner-only). "
    "Legacy WEIR_REFINER_WORK_TEMP_STALE_SWEEP_SCHEDULE_ENABLED and "
    "WEIR_REFINER_WORK_TEMP_STALE_SWEEP_SCHEDULE_INTERVAL_SECONDS still apply to **both** scopes when the "
    ...
    "Minimum age before deletion (shared narrow exception): WEIR_REFINER_WORK_TEMP_STALE_SWEEP_MIN_STALE_AGE_SECONDS "
...
_MOVIE_OUTPUT_CLEANUP_NOTE = (
    ...
    "Before any delete, Weir asks every media manager connected for Movies which library files it still "
...
_TV_OUTPUT_CLEANUP_NOTE = (
    ...
    "Before any delete, Weir asks every media manager connected for TV episodes which library files it still "
...
_FAILURE_CLEANUP_NOTE = (
    ...
    "WEIR_REFINER_MOVIE_FAILURE_CLEANUP_SCHEDULE_INTERVAL_SECONDS / "
    "WEIR_REFINER_MOVIE_FAILURE_CLEANUP_GRACE_PERIOD_SECONDS and "
    "WEIR_REFINER_TV_FAILURE_CLEANUP_SCHEDULE_ENABLED / "
    "WEIR_REFINER_TV_FAILURE_CLEANUP_SCHEDULE_INTERVAL_SECONDS / "
    "WEIR_REFINER_TV_FAILURE_CLEANUP_GRACE_PERIOD_SECONDS. "
...
_WATCHED_FOLDER_SCAN_PERIODIC_NOTE = (
    ...
    "WEIR_REFINER_WATCHED_FOLDER_REMUX_SCAN_DISPATCH_PERIODIC_ENQUEUE_REMUX_JOBS (default on). "
    "Restart the API after changing that one — it is read at process start only."
)
_REFINER_PROBE_NOTE = (
    "Refiner ffprobe preflight depth: WEIR_REFINER_PROBE_SIZE_MB and "
    "WEIR_REFINER_ANALYZE_DURATION_SECONDS in apps/backend/.env (read at startup; restart required)."
)


def refiner_runtime_settings_from_settings(settings: WeirSettings) -> RefinerRuntimeSettingsOut:
    ...
    if disabled:
        summary = (
            ...
            "WEIR_REFINER_WORKER_COUNT to at least 1 and restart this API."
        )
    elif n == 1:
        summary = (
        ...
        summary = (
            ...
            "those refiner_jobs slots may process files at once."
        )
