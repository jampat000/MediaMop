...
/// </summary>
public sealed class RefinerWatchedFolderScanDispatchScheduleTaskTests
{
    ...
    public async Task With_the_global_env_kill_switch_off_no_periodic_scan_is_ever_enqueued_even_though_everything_else_is_ready()
    {
        ...
        // only WEIR_REFINER_WATCHED_FOLDER_REMUX_SCAN_DISPATCH_SCHEDULE_ENABLED stands in the way.
        using var store = new StoreFixture(
            ("WEIR_CREDENTIALS_SECRET", "schedule-task-tests-secret-kill"),
            ("WEIR_REFINER_WATCHED_FOLDER_REMUX_SCAN_DISPATCH_SCHEDULE_ENABLED", "0"));
        ...
        var jobStore = new RefinerJobStore(store.Database, store.Clock);
        var task = new RefinerWatchedFolderScanDispatchScheduleTask(store.Database, store.Options, jobStore, store.Clock, NullLogger<RefinerWatchedFolderScanDispatchScheduleTask>.Instance);
        ...
        var count = await store.Scalar("SELECT COUNT(*) FROM refiner_jobs WHERE job_kind = 'refiner.watched_folder.remux_scan_dispatch.v1'");
    ...
    public async Task A_disabled_library_is_never_scheduled_regardless_of_the_scope_switch()
    {
        ...
        await using (var uow = await UnitOfWork.OpenAsync(store.Database))
        {
            ...
            await LibraryStore.UpdateAsync(uow, library, new RefinerLibraryInput { Name = library.Name, MediaType = library.MediaType, WatchedFolder = watched, OutputFolder = output, Enabled = false });
            await uow.CommitAsync();
        }
    ...
    public async Task Manual_enqueue_works_even_while_the_scope_schedule_switch_is_off()
    {
        ...
        var jobStore = new RefinerJobStore(store.Database, store.Clock);
        await using var uow = await UnitOfWork.OpenAsync(store.Database);
        var job = await RefinerWatchedFolderScanDispatchEnqueue.EnqueueScanDispatchJobAsync(uow, jobStore, enqueueRemuxJobs: true, "manual", "movie", libraryId);
        await uow.CommitAsync();
