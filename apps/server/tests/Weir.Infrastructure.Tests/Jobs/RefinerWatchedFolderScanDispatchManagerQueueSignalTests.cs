...
/// <c>test_refiner_watched_folder_remux_scan_dispatch_evaluate.py</c> and
/// <c>test_refiner_watched_folder_remux_scan_dispatch_lane.py</c>), driven through
/// <see cref="RefinerWatchedFolderScanDispatchJobHandler"/> with a fake Radarr behind
/// <see cref="FakeManagerHttp"/> rather than a hand-built signal.
/// </summary>
public sealed class RefinerWatchedFolderScanDispatchManagerQueueSignalTests
{
    private static async Task<(StoreFixture Store, RefinerJobStore Jobs, RefinerWatchedFolderScanDispatchJobHandler Handler, FakeManagerHttp Http, MediaManagerConnectionService Connections)> BuildAsync()
    {
    ...
    private static async Task RunScanAsync(RefinerWatchedFolderScanDispatchJobHandler handler, RefinerJobStore jobs, long libraryId)
    {
    ...
    public async Task A_file_a_linked_manager_still_reports_downloading_is_not_enqueued()
    {
        ...
        Assert.NotNull(file);
        Assert.Equal(RefinerFileStatuses.BlockedUpstream, file!.Status);
