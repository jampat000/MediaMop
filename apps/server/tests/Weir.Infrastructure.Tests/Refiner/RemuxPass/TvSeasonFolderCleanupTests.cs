...
/// <summary>
/// Ported from <c>apps/backend/tests/test_refiner_tv_season_folder_cleanup.py</c>: the manager-signal, active-job,
...
public sealed class TvSeasonFolderCleanupTests : IDisposable
{
    ...
    private static async Task<(StoreFixture Store, TvSeasonFolderCleanup Cleanup, FakeManagerHttp Http, MediaManagerConnectionService Connections, RefinerJobStore Jobs)> BuildAsync()
    {
    ...
    public async Task Blocked_by_active_other_tv_job_for_the_same_path()
    {
        ...
        await jobs.EnqueueOrGetAsync("k1", "refiner.file.remux_pass.v1", PyJsonWriter.Dumps(payload, PyJsonFormat.Compact));
    ...
    public async Task Not_blocked_by_a_movie_scope_job_at_the_same_path_and_deletes_the_season()
    {
        ...
        await jobs.EnqueueOrGetAsync("km", "refiner.file.remux_pass.v1", PyJsonWriter.Dumps(payload, PyJsonFormat.Compact));
    ...
    public async Task A_prior_recorded_activity_success_clears_a_never_finished_episode()
    {
        ...
        await store.Execute(
            "INSERT INTO activity_events (event_type, module, title, detail) VALUES ('refiner.file_remux_pass_completed', 'refiner', 't', '" +
