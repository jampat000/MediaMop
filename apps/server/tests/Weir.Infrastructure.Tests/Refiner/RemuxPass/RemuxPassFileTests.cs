...
/// <summary>Ported from <c>apps/backend/tests/test_refiner_output_collision.py</c>, on real temporary folders.</summary>
...
/// <summary>Ported from <c>apps/backend/tests/test_refiner_sidecar_migration.py</c>.</summary>
...
/// <summary>Ported from the access and writer tests of <c>apps/backend/tests/test_refiner_file_settling.py</c>, plus the pass's read guard.</summary>
public sealed class SourceFilesTests : IDisposable
{
    ...
    private static RefinerLibraryRecord Library(bool skip = false) => new() { Name = "Movies", SkipAccessTests = skip };
...
/// <summary>Ported from <c>apps/backend/tests/test_refiner_file_remux_pass_paths.py</c> and the library-folder rules.</summary>
public sealed class RemuxPassPathsTests : IDisposable
{
    ...
    {
        RefinerLibraryRecord Library(string watched, string output, string work = "") => new() { Name = "Movies", WatchedFolder = watched, OutputFolder = output, WorkFolder = work };
        ...
        Assert.Equal(Path.Join(_root.Path, "refiner", "refiner-movie-work"), runtime.WorkFolderEffective);
    }
