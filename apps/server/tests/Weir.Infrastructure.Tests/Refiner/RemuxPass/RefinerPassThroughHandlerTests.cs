...
/// <summary>
/// Ported from <c>apps/backend/tests/test_refiner_pass_through.py</c>: <see cref="RefinerPassThroughHandler"/> against a
/// real database and real temporary folders, with the manager side behind <see cref="FakeManagerHttp"/>.
/// </summary>
public sealed class RefinerPassThroughHandlerTests : IDisposable
{
    ...
    private RefinerPassThroughHandler Handler() =>
        new(_fixture.Store.Database, TimeProvider.System, NullLogger<RefinerPassThroughHandler>.Instance, _fixture.Reporter);
    ...
    private async Task<long> LibraryAsync(string collision = "replace")
    {
        ...
        return Convert.ToInt64(await _fixture.Db(uow => uow.ExecuteScalarWriteAsync(
            "INSERT INTO refiner_libraries (name, media_type, watched_folder, output_folder, work_folder, output_collision_policy, display_order) " +
    ...
    private Task FileRowAsync(long libraryId, string relativePath, string status = "processing_failed") =>
        _fixture.Store.Execute(
            $"INSERT INTO refiner_files (library_id, relative_path, status, status_reason) VALUES ({libraryId}, '{relativePath}', '{status}', 'failed')");
    ...
    public async Task The_original_is_delivered_byte_for_byte_and_the_source_survives()
    {
        ...
        Assert.Equal("passed_through", await ScalarText("SELECT status FROM refiner_files"));
        Assert.Equal(1, await _fixture.Store.Scalar("SELECT count(*) FROM activity_events WHERE event_type = 'refiner.file_passed_through'"));
        var detail = (PyDict)PyJsonParser.Parse(await ScalarText("SELECT detail FROM activity_events WHERE event_type = 'refiner.file_passed_through'"));
    ...
    public async Task Nothing_half_written_is_left_when_the_source_vanishes_mid_flight()
    {
        ...
        Assert.Equal(1, await _fixture.Store.Scalar("SELECT count(*) FROM activity_events WHERE event_type = 'refiner.file_pass_through_failed'"));
        var detail = (PyDict)PyJsonParser.Parse(await ScalarText("SELECT detail FROM activity_events WHERE event_type = 'refiner.file_pass_through_failed'"));
        Assert.True(((PyBool)detail["source_kept"]).Value);
    ...
    public async Task Repeated_pass_through_jobs_for_the_same_file_share_one_dedupe_key()
    {
        ...
        var first = await jobs.EnqueueOrGetAsync($"refiner.file.pass_through.v1:{library}:x.mkv", IntakeRules.PassThroughJobKind, body);
        var second = await jobs.EnqueueOrGetAsync($"refiner.file.pass_through.v1:{library}:x.mkv", IntakeRules.PassThroughJobKind, body);
