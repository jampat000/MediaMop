...
/// <c>test_refiner_watched_folder_watcher.py</c>, plus the two behaviours issue #552 asks for beyond
...
/// </summary>
public sealed class RefinerWatchedFolderWatcherServiceTests
{
    private const string ScanJobKind = RefinerWatchedFolderScanDispatchJobKinds.ScanDispatch;
        ...
        StoreFixture store, string watched, string output, bool fileSystemEventsEnabled = true)
    {
        ...
        var seeded = await LibraryStore.SeededForScopeAsync(uow, RefinerMediaScopes.Movie) ?? throw new InvalidOperationException("No seeded Movies library.");
        var updated = await LibraryStore.UpdateAsync(uow, seeded, new RefinerLibraryInput
        {
            Name = seeded.Name,
    ...
    private static List<string> ScanJobPayloads(StoreFixture store)
    {
        ...
        command.CommandText = "SELECT payload_json FROM refiner_jobs WHERE job_kind = @kind";
    ...
    private static RefinerWatchedFolderWatcherService Service(StoreFixture store, WatcherStateStore state) =>
        new(store.Database, store.Options, new RefinerJobStore(store.Database, TimeProvider.System), state,
            TimeProvider.System, NullLogger<RefinerWatchedFolderWatcherService>.Instance);
    ...
    public async Task A_file_appearing_becomes_a_candidate_without_waiting_for_the_scan_interval()
    {
        using var store = new StoreFixture(
            ...
            ("WEIR_REFINER_WATCHER_DEBOUNCE_SECONDS", "1"));
    ...
    public async Task An_overflow_or_error_immediately_queues_a_full_scan_and_restarts_the_watcher()
    {
        using var store = new StoreFixture(
            ...
            ("WEIR_REFINER_WATCHER_DEBOUNCE_SECONDS", "60"));
        var watched = store.Home.Join("watch");
        var output = store.Home.Join("out");
        Directory.CreateDirectory(watched);
        Directory.CreateDirectory(output);
        ...
        var service = new FakeWatcherService(store.Database, store.Options, new RefinerJobStore(store.Database, TimeProvider.System),
            new WatcherStateStore(), TimeProvider.System, NullLogger<RefinerWatchedFolderWatcherService>.Instance);
        await service.StartAsync(CancellationToken.None);
        try
        {
    ...
    private sealed class FakeWatcherService(
        SqliteDatabase database, Weir.Core.Configuration.WeirOptions options, RefinerJobStore jobStore, WatcherStateStore state, TimeProvider time,
        Microsoft.Extensions.Logging.ILogger<RefinerWatchedFolderWatcherService> logger)
        : RefinerWatchedFolderWatcherService(database, options, jobStore, state, time, logger)
    {
