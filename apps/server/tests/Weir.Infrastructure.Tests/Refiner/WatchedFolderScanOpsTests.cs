...
/// <c>test_refiner_watched_folder_remux_scan_dispatch_ops.py</c>.</summary>
public sealed class WatchedFolderScanOpsTests
{
    ...
    {
        db.Execute(
            "INSERT INTO refiner_jobs (dedupe_key, job_kind, payload_json, status, max_attempts, attempt_count) " +
            "VALUES (@dedupe, @kind, @payload, @status, 3, @attempts)",
            ("@dedupe", "wf-" + Guid.NewGuid().ToString("N")),
            ("@kind", "refiner.file.remux_pass.v1"),
            ("@payload", payloadJson),
            ("@status", status),
    ...
    public async Task Completed_remux_output_blocks_repeat_scan_when_source_cleanup_failed()
    {
        ...
        var detail = $$"""
            ...
            """.Replace("\n", string.Empty);
        db.Execute(
            "INSERT INTO activity_events (module, event_type, title, detail) VALUES ('refiner', 'refiner.file_remux_pass_completed', 'x', @detail)",
            ("@detail", detail));
    ...
    {
        var library = new RefinerLibraryRecord { Name = "Movies", MediaType = RefinerMediaScopes.Movie, WatchedFolder = string.Empty, OutputFolder = string.Empty };
    ...
    public void Resolve_path_runtime_succeeds_for_a_library_with_real_watched_and_output_folders()
    {
        ...
        var library = new RefinerLibraryRecord { Name = "Movies", MediaType = RefinerMediaScopes.Movie, WatchedFolder = watched, OutputFolder = output, WorkFolder = string.Empty };
