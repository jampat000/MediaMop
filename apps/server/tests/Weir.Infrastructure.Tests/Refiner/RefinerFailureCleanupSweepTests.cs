...
/// <summary>
/// Ported from <c>apps/backend/tests/test_refiner_failure_cleanup.py</c>: <see cref="RefinerFailureCleanupSweep"/>
/// against a real database and real temporary folders, with each media manager's queue behind
/// <see cref="FakeManagerHttp"/>.
/// </summary>
public sealed class RefinerFailureCleanupSweepTests : IDisposable
{
    ...
    private RefinerFailureCleanupSweep Sweep() =>
        new(_fixture.Store.Database, _fixture.Store.Options, _fixture.Connections, TimeProvider.System, NullLogger<RefinerFailureCleanupSweep>.Instance);
    ...
    /// Python's <c>seed_refiner_libraries</c>.</summary>
    private async Task SeedLibrariesAsync(string movieWatched, string movieOutput, string movieWork, string tvWatched, string tvOutput, string tvWork)
    {
        ...
        await _fixture.Db(uow => uow.ExecuteAsync(
            "INSERT INTO refiner_libraries (name, media_type, watched_folder, output_folder, work_folder, display_order) VALUES ('Movies', 'movie', $w, $o, $k, 1)",
            ("$w", movieWatched), ("$o", movieOutput), ("$k", movieWork)));
        await _fixture.Db(uow => uow.ExecuteAsync(
            "INSERT INTO refiner_libraries (name, media_type, watched_folder, output_folder, work_folder, display_order) VALUES ('TV', 'tv', $w, $o, $k, 2)",
    ...
    private Task<int> AddFailedJobAsync(string rel, string scope, bool dryRun = false, TimeSpan? age = null) =>
        _fixture.Db(uow => uow.ExecuteAsync(
            "INSERT INTO refiner_jobs (dedupe_key, job_kind, payload_json, status, updated_at) VALUES ($dedupe, 'refiner.file.remux_pass.v1', $payload, 'failed', $updated)",
    ...
    private Task<int> AddPendingJobAsync(string rel, string scope) =>
        _fixture.Db(uow => uow.ExecuteAsync(
            "INSERT INTO refiner_jobs (dedupe_key, job_kind, payload_json, status) VALUES ($dedupe, 'refiner.file.remux_pass.v1', $payload, 'pending')",
    ...
    public async Task A_failed_movie_older_than_grace_cleans_source_output_and_temp()
    {
        ...
        var temp = Path.Combine(mwork, "Film.refiner.tmp.mkv");
    ...
    public async Task Missing_path_settings_skip_cleanly()
    {
        await _fixture.Store.Execute("DELETE FROM refiner_libraries");
        await _fixture.Db(uow => uow.ExecuteAsync(
            "INSERT INTO refiner_libraries (name, media_type, display_order) VALUES ('Movies', 'movie', 1)"));
    ...
    {
        var libraryIds = await _fixture.Db(uow => uow.QueryAsync("SELECT id FROM refiner_libraries", reader => reader.GetInt64(0)));
        foreach (var libraryId in libraryIds)
        {
            await _fixture.Store.Execute($"INSERT INTO refiner_library_manager_links (library_id, connection_id) VALUES ({libraryId}, {connectionId})");
        }
    }
}
