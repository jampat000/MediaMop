...
/// <summary>Ported from <c>apps/backend/tests/test_refiner_queue_adapter.py</c> (movie/TV dialect mapping).</summary>
public sealed class QueueRowMappingTests
{
    ...
    public void Movie_active_downloading_row_path_match_owns_and_blocks()
    {
        ...
        var v = QueueRowMapping.MapQueueRowToRefinerView(row, QueueRowMapping.MovieDialect, candidatePath: "D:/media/film.mkv");
        ...
        Assert.True(RefinerDomain.FileIsOwnedByQueue([v]));
        Assert.True(RefinerDomain.ShouldBlockForUpstream([v]));
    }
    ...
    public void Movie_import_pending_row_owns_by_path_suppression_carried()
    {
        ...
        var v = QueueRowMapping.MapQueueRowToRefinerView(row, QueueRowMapping.MovieDialect, candidatePath: "/data/complete/movie.mkv");
        Assert.True(v.IsImportPending);
        Assert.False(v.IsUpstreamActive);
        Assert.True(v.BlockingSuppressedForImportWait);
        Assert.True(RefinerDomain.FileIsOwnedByQueue([v]));
        Assert.False(RefinerDomain.ShouldBlockForUpstream([v]));
    }
    ...
    public void Movie_downloading_row_suppressed_does_not_block_but_still_owns()
    {
        ...
        var v = QueueRowMapping.MapQueueRowToRefinerView(row, QueueRowMapping.MovieDialect, candidatePath: "/srv/queue/x.mkv");
        Assert.True(v.IsUpstreamActive);
        Assert.True(v.BlockingSuppressedForImportWait);
        Assert.True(RefinerDomain.FileIsOwnedByQueue([v]));
        Assert.False(RefinerDomain.ShouldBlockForUpstream([v]));
    ...
    public void Movie_completed_row_owns_via_title_year_anchor_only()
    {
        ...
        var v = QueueRowMapping.MapQueueRowToRefinerView(row, QueueRowMapping.MovieDialect);
        ...
        Assert.True(RefinerDomain.FileIsOwnedByQueue([v], cand));
        Assert.False(RefinerDomain.ShouldBlockForUpstream([v], cand));
    }
    ...
    public void Movie_applicability_via_movie_id_without_path()
    {
        ...
        var v = QueueRowMapping.MapQueueRowToRefinerView(row, QueueRowMapping.MovieDialect, candidatePath: null, candidateEntityId: 42);
        Assert.True(v.AppliesToFile);
        Assert.True(v.IsUpstreamActive);
    }
    ...
    public void Tracked_download_status_fallback()
    {
        ...
        var v = QueueRowMapping.MapQueueRowToRefinerView(row, QueueRowMapping.MovieDialect, candidatePath: "/tmp/z.mkv");
        Assert.True(v.IsUpstreamActive);
    }
    ...
    public void Tv_active_paused_row_series_id_owns_and_blocks()
    {
        ...
        var v = QueueRowMapping.MapQueueRowToRefinerView(row, QueueRowMapping.TvDialect, candidateEntityId: 9001);
        Assert.True(v.AppliesToFile);
    ...
    public void Tv_sparse_missing_series_uses_top_level_title_for_anchor()
    {
        ...
        var v = QueueRowMapping.MapQueueRowToRefinerView(row, QueueRowMapping.TvDialect);
        Assert.False(v.AppliesToFile);
    ...
    public void Mixed_scope_mapped_rows_domain_aggregation()
    {
        ...
        var cBusy = QueueRowMapping.MapQueueRowToRefinerView(movieBusy, QueueRowMapping.MovieDialect, candidatePath: "c:/q/a.mkv");
        var cSparse = QueueRowMapping.MapQueueRowToRefinerView(tvSparse, QueueRowMapping.TvDialect);
        var cand = new FileAnchorCandidate("Alien 1979");
        var rows = new[] { cBusy, cSparse };
        Assert.True(RefinerDomain.FileIsOwnedByQueue(rows, cand));
        Assert.True(RefinerDomain.ShouldBlockForUpstream(rows, cand));
    }
    ...
    public void Neutral_media_block_and_entity_id_drive_a_movie_row()
    {
        ...
        var v = QueueRowMapping.MapQueueRowToRefinerView(row, QueueRowMapping.MovieDialect, candidateEntityId: 7);
        Assert.True(v.AppliesToFile);
    ...
    public void Neutral_media_block_drives_a_tv_row()
    {
        ...
        var v = QueueRowMapping.MapQueueRowToRefinerView(row, QueueRowMapping.TvDialect, candidateEntityId: 11);
        Assert.True(v.AppliesToFile);
...
/// <summary>Ported from <c>apps/backend/tests/test_refiner_candidate_gate_evaluate.py</c> (domain evaluation, no HTTP).</summary>
