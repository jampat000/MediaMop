...
namespace Weir.Core.Tests.Refiner.RemuxPass;

/// <summary>Ported from <c>apps/backend/tests/test_refiner_file_remux_pass_visibility.py</c>.</summary>
public sealed class RemuxPassVisibilityTests
{
    ...
    public void The_detail_is_compact_ascii_json_with_removed_tracks_and_counts()
    {
        ...
        Assert.Contains("\"module\":\"refiner\",\"action\":\"remux\",\"trigger\":\"worker\",\"result\":\"success\"", detail, StringComparison.Ordinal);
        Assert.Equal(
...
/// <summary>Ported from the policy half of <c>test_refiner_requeue_service.py</c> and <c>refiner_failure_classes.py</c>.</summary>
public sealed class FailureClassesTests
{
    private static readonly DateTimeOffset Now = new(2026, 8, 29, 12, 0, 0, TimeSpan.Zero);

    private static RefinerLibraryRecord Library(long maxAttempts = 3, long backoff = 300, bool retryExecution = true, bool retryPreflight = false) => new()
    {
        Name = "Movies",
    ...
    public void Outcomes_map_onto_the_retry_vocabulary(string? outcome, string expected) =>
        Assert.Equal(expected, RefinerFailureClasses.Classify(outcome));

    [Fact]
    public void Guardrail_and_unknown_are_never_retryable()
    {
        Assert.False(RefinerFailureClasses.IsRetryable("guardrail", true, true));
        Assert.False(RefinerFailureClasses.IsRetryable("unknown", true, true));
        Assert.True(RefinerFailureClasses.IsRetryable("execution", false, true));
        Assert.False(RefinerFailureClasses.IsRetryable("preflight", false, true));
    }

    [Fact]
    public void Backoff_doubles_from_the_base_and_caps_at_an_hour()
    {
        Assert.Equal(300, RefinerFailureClasses.BackoffSecondsForAttempt(1, 300));
        Assert.Equal(600, RefinerFailureClasses.BackoffSecondsForAttempt(2, 300));
        Assert.Equal(3600, RefinerFailureClasses.BackoffSecondsForAttempt(5, 300));
        Assert.Equal(3600, RefinerFailureClasses.BackoffSecondsForAttempt(40, 1));
        Assert.Equal(1, RefinerFailureClasses.BackoffSecondsForAttempt(0, 0));
    }
...
/// <summary>Ported from the size-settling tests of <c>apps/backend/tests/test_refiner_file_settling.py</c>.</summary>
public sealed class FileSettlingTests
{
    private static readonly DateTimeOffset Now = new(2026, 8, 29, 12, 0, 0, TimeSpan.Zero);

    private static RefinerLibraryRecord Library(long interval = 30, bool ignore = false) =>
...
/// <summary>Ported from <c>apps/backend/tests/test_refiner_manager_library_truth.py</c>.</summary>
...
/// <summary>What a pass reads off the probe (<c>run.py</c> and <c>refiner_runner_units.py</c> helpers).</summary>
