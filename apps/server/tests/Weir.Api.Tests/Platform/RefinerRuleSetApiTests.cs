...
/// (<c>POST</c>/<c>PUT /api/v1/refiner/rule-sets</c>) and their validation: an unknown track name
...
/// </summary>
public sealed class RefinerRuleSetApiTests
{
    ...
    public async Task Updating_a_rule_set_round_trips_every_new_field()
    {
        ...
        using var listResponse = await client.GetAsync("/api/v1/refiner/rule-sets");
    ...
    public async Task An_unknown_enum_value_is_refused_with_422(string field, string value)
    {
        ...
        using var response = await client.PostAsync("/api/v1/refiner/rule-sets", body);
