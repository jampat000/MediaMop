...
/// <summary>
/// End-to-end proof of the fix for #530 at the HTTP layer: <c>GET /refiner/files</c> must answer 200 for a
/// row at <c>passed_through</c> or <c>rejected</c>, and the <c>file_status</c> query filter must accept
/// either value rather than 422. Also covers the everyday Refiner Files list shape.
/// </summary>
public sealed class RefinerFilesApiTests
{
    private static async Task<long> SeedLibraryAsync(WeirTestServer server, string name = "Movies bug530") =>
        await TestDatabase.ScalarAsync(
            server,
            "INSERT INTO refiner_libraries (name, media_type) VALUES ($name, 'movie') RETURNING id",
            ("$name", name));
    ...
    public async Task Filtering_by_either_bug_530_status_answers_200_not_422(string status)
    {
        ...
        using var response = await client.GetAsync($"/api/v1/refiner/files?file_status={status}");
    ...
    public async Task A_page_with_a_passed_through_file_alongside_others_answers_200_not_500()
    {
        ...
        using var response = await client.GetAsync("/api/v1/refiner/files");
    ...
    public async Task An_invalid_file_status_is_still_rejected_with_422()
    {
        ...
        using var response = await client.GetAsync("/api/v1/refiner/files?file_status=not_a_real_status");
    ...
    public async Task Two_libraries_with_overlapping_folders_are_refused_with_400()
    {
        ...
        await client.PostAsync("/api/v1/refiner/libraries", new { csrf_token = csrf, name = "Anime", media_type = "movie", watched_folder = @"c:\anime-in", output_folder = @"c:\anime-out" });
