...
/// (<c>WEIR_REFINER_WORKER_COUNT=0</c>), so an ffmpeg call is not expected, but is answered harmlessly if made.</summary>
...
/// <summary>
/// End-to-end HTTP proof of issue #501: <c>GET /refiner/files/{id}/tracks</c> lists a fresh probe with the rules'
/// verdict per stream, and <c>POST /refiner/files/{id}/manual-plan</c> validates and queues an operator's choice.
/// </summary>
public sealed class RefinerManualTrackApiTests
{
    private static async Task<(WeirTestServer Server, ApiTestClient Client, long FileId, string RelativePath)> StartWithHeldFileAsync()
    {
        var server = await WeirTestServer.StartAsync(
            [("WEIR_SESSION_SECRET", ApiTestClient.Secret), ("WEIR_REFINER_WORKER_COUNT", "0")],
        ...
        var libraryId = await TestDatabase.ScalarAsync(
            server,
            "INSERT INTO refiner_libraries (name, media_type, watched_folder, output_folder) VALUES ($name, 'movie', $watched, $output) RETURNING id",
            ("$name", "Movies #501"), ("$watched", watched), ("$output", output));
        var fileId = await TestDatabase.ScalarAsync(
            server,
            "INSERT INTO refiner_files (library_id, relative_path, status, last_seen_at) VALUES ($lib, $path, 'on_hold', CURRENT_TIMESTAMP) RETURNING id",
    ...
    public async Task Getting_tracks_requires_sign_in()
    {
        ...
        using var response = await anonymous.GetAsync($"/api/v1/refiner/files/{fileId}/tracks");
    ...
    public async Task Getting_tracks_returns_every_stream_with_the_rules_verdict()
    {
        ...
        using var response = await client.GetAsync($"/api/v1/refiner/files/{fileId}/tracks");
    ...
    public async Task Manual_plan_needs_an_operator_role()
    {
        ...
        using var response = await viewer.PostAsync(
            $"/api/v1/refiner/files/{fileId}/manual-plan",
    ...
    public async Task A_valid_manual_plan_is_queued_and_recorded_as_activity()
    {
        ...
        var jobCount = await TestDatabase.ScalarAsync(server, "SELECT COUNT(*) FROM refiner_jobs WHERE job_kind = 'refiner.file.remux_pass.v1'");
        Assert.Equal(1, jobCount);
        var activityCount = await TestDatabase.ScalarAsync(
            server, "SELECT COUNT(*) FROM activity_events WHERE event_type = 'refiner.file_manual_plan_queued'");
