...
/// <summary>
/// HTTP-level proof of issue #502's preview endpoint: <c>POST /api/v1/refiner/libraries/{id}/preview</c>
...
/// </summary>
public sealed class RefinerRulesPreviewApiTests
{
        ...
        ApiTestClient client, string watched, string output, string primaryAudioLang)
    {
        ...
        var linked = await client.PutAsync(
            $"/api/v1/refiner/libraries/{libraryId}",
    ...
    public async Task Unsaved_rules_change_which_track_is_kept_and_the_database_stays_the_same()
    {
        ...
        var ruleSetCountBefore = await TestDatabase.ScalarAsync(server, "SELECT COUNT(*) FROM refiner_rule_sets");
        ...
        var storedPrimaryLang = await TestDatabase.ScalarStringAsync(server, "SELECT primary_audio_lang FROM refiner_rule_sets WHERE id = $id", ("$id", ruleSetId));
        Assert.Equal("jpn", storedPrimaryLang);
        var ruleSetCount = await TestDatabase.ScalarAsync(server, "SELECT COUNT(*) FROM refiner_rule_sets");
    ...
    public async Task Neither_path_nor_both_paths_given_is_refused_with_400()
    {
        ...
        using var neither = await client.PostAsync($"/api/v1/refiner/libraries/{libraryId}/preview", new { csrf_token = await client.CsrfAsync() });
    ...
    public async Task A_preview_creates_no_job_rows_and_writes_nothing()
    {
        ...
        var (libraryId, _) = await SeedLibraryWithRuleSetAsync(client, watched, output, primaryAudioLang: "eng");
        await File.WriteAllBytesAsync(Path.Join(watched, "movie.mkv"), new byte[16]);
        var jobsBefore = await TestDatabase.ScalarAsync(server, "SELECT COUNT(*) FROM refiner_jobs");
        var filesBefore = await TestDatabase.ScalarAsync(server, "SELECT COUNT(*) FROM refiner_files");
        ...
        Assert.Equal(jobsBefore, await TestDatabase.ScalarAsync(server, "SELECT COUNT(*) FROM refiner_jobs"));
