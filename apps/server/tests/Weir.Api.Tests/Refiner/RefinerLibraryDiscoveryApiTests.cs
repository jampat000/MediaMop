...
namespace Weir.Api.Tests.Refiner;

/// <summary>
/// Media-manager library discovery, import, drift and unlink over real HTTP against a scripted Deluno
/// manifest (port of the HTTP-relevant assertions in <c>test_refiner_library_discovery.py</c>; #554).
/// </summary>
public sealed class RefinerLibraryDiscoveryApiTests
{
    private const string Connections = "/api/v1/media-managers/connections";

    private static string DiscoverPath(long connectionId) => $"/api/v1/refiner/libraries/discover/{connectionId}";

    private static string ImportPath(long connectionId) => $"/api/v1/refiner/libraries/discover/{connectionId}/import";

    private static string DriftPath(long connectionId) => $"/api/v1/refiner/libraries/discover/{connectionId}/drift";

    private static string UnlinkPath(long libraryId) => $"/api/v1/refiner/libraries/{libraryId}/unlink";
    ...
    private static async Task<(WeirTestServer Server, ApiTestClient Client, ScriptedManager Manager)> StartAsync()
    {
        ...
        var server = await WeirTestServer.StartAsync(
            [("WEIR_SESSION_SECRET", Secret), ("WEIR_REFINER_WORKER_COUNT", "0")],
    ...
    public async Task Resync_reports_a_moved_root_and_changes_nothing()
    {
        ...
        try
        {
            ...
            using var library = await client.GetAsync($"/api/v1/refiner/libraries/{libraryId}");
    ...
    public async Task Resync_reports_a_library_that_appeared()
    {
        ...
        try
        {
            ...
            using var libraries = await client.GetAsync("/api/v1/refiner/libraries");
    ...
    public async Task Unlinking_keeps_the_library()
    {
        ...
        try
        {
            ...
            using var reloaded = await client.GetAsync($"/api/v1/refiner/libraries/{libraryId}");
