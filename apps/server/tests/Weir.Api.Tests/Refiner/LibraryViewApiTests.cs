...
public sealed class LibraryViewApiTests
{
    ...
    private static string OverviewPath(long libraryId) => $"/api/v1/refiner/libraries/{libraryId}/library-overview";

    private static string ProblemsPath(long libraryId) => $"/api/v1/refiner/libraries/{libraryId}/library-problems";

    private static string FilesPath(long libraryId, string query = "") =>
        $"/api/v1/refiner/libraries/{libraryId}/library-files" + (query.Length > 0 ? "?" + query : "");
    ...
    public async Task A_library_that_allows_hardlinked_files_stops_reporting_seeding_as_a_problem()
    {
        ...
        using var saved = await client.PutAsync(
            $"/api/v1/refiner/libraries/{libraryId}/library-settings",
