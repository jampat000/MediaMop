...
public sealed class PeriodicTasksStartupTests
{
    ...
    public async Task The_server_hosts_the_python_lifespan_loops_after_startup_recovery()
    {
        ...
        Assert.Equal(
            ["auth-session-cleanup", "platform-job-rows-retention", "processing-file-log-retention", "processing-watched-folder-remux-scan-dispatch-enqueue", "suite-configuration-backup", "suite-log-retention"],
