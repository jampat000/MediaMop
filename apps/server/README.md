...
### Seams for #507, #508 and #509 (separate work)

- **`ILibraryFileChangeNotifier`** (`Weir.Core/MediaManagers/LibraryFileChangeRules.cs`) is #507's own interface (`LibraryFileChange`, the notifier, and its pure rules), copied here verbatim from branch `feat/507-file-changed` so the two branches merge onto the same seam without a rename. `LibraryCleanHandler` calls it once per committed swap; `Weir.Infrastructure.LibraryMode.NoOpLibraryFileChangeNotifier` is the default registration until that branch's own `LibraryFileChangeNotifier` lands and replaces it. A first draft of this seam lived directly on `IMediaManagerPort` with a different shape (`ListLibraryFilesAsync`/`NotifyFileChangedAsync`); it was removed once #507's actual interface (a *different* `IMediaManagerPort` extension: `Task<ManagerLibraryFilesSignal> ListLibraryFilesAsync(connection, mediaScope, ct)` and `Task<ManagerNotifyOutcome> FileChangedAsync(connection, advertisedCapabilities, titleId, filePath, reason, ct)`) was confirmed, so #505 never ships an incompatible seam of its own.
- **Manager title matching** ("Blade Runner 2049 (Radarr)") needs that same `ListLibraryFilesAsync` port extension. Until #507 merges, `LibraryScanHandler` reports every file unmatched (`manager_kind`/`manager_title` are always `null`) — #505 explicitly allows this ("An unmatched file is still processable"). Wiring it up is a small follow-up once #507 lands: call it per linked connection in `LibraryScanHandler`, same as the notify call in `LibraryCleanHandler`.
- **#508 (seeding / re-download-risk warnings)**: not built here. `LibraryModeEndpoints.ConfirmationRequired`'s JSON already carries an (empty) `warnings` array, and the web's `LibraryRemovalConfirmationDialog` already renders whatever list it is given, so #508's `LibraryCleanPreflight` can populate both without a shape change.
- **#509 (re-download after a rule change)**: not built here. The rule-change rescan (above) only ever re-plans; nothing here stores which tracks a past clean removed, which #509 needs to compute "12 titles are missing tracks your new rules keep".

### Web

`apps/web/src/pages/refiner/refiner-library-section.tsx` (a new "Library" tab on the Refiner page): library folders (add/remove, saved via `PUT .../library-settings`), the schedule On/Off toggle, a "Scan now" button, and the file table (classification/manager/search filters, per-file summary, multi-select, "Clean selected"). `apps/web/src/components/refiner/library-removal-confirmation-dialog.tsx` renders the exact required sentence — "N files, M tracks will be removed. Removed tracks are gone for good; getting one back means downloading the title again." — plus the estimated size saved and (empty today) risk warnings, shared by Clean and by turning the schedule on. `apps/web/src/lib/refiner/library-api.ts` treats the `confirm_final_removal_required` 400 as a normal, typed outcome (not a thrown error), so the caller decides whether to show the dialog. New OpenAPI paths/schemas were hand-added to the committed `apps/web/openapi/weir-openapi.json` (`npm run api:types:generate` regenerates the TS types from it): the spec's normal source, `scripts/export-openapi.py`, introspects the **Python** FastAPI app, which has no library-mode routes to discover, so these are the first hand-written entries in that file.

### Tests

`Weir.Core.Tests/LibraryMode` (`LibraryFilePlannerTests`, `LibraryModeSettingsTests`) cover classification and folder/JSON rules in isolation. `Weir.Infrastructure.Tests/LibraryMode` (`LibraryScanHandlerTests`, `LibraryCleanHandlerTests`) run both handlers against a real database and real files behind a fake ffmpeg/ffprobe process runner: a scan never writes to a file; clean is refused without confirmation; a library failure never queues a reject or pass-through job; library jobs are claimed behind a pending download job; an unmatched file still processes; and a full clean commits the swap and leaves no temp/backup leftovers. `apps/web/src/pages/refiner/refiner-library-section.test.tsx` covers the file list rendering and the confirmation dialog's exact text end to end. No contract-suite scenario was added: the Python-driven contract suite has no library-mode surface to exercise against a running server of either backend.
