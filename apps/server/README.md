...
- **Clean jobs** (`job_kind = "processing.library.clean.v1"`, dedupe key `…:{library_id}:{sha256(path)}`) are ordinary, real jobs: one per file, claimed, run and completed exactly like a download-pipeline job.
