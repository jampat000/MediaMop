...
- **Manager title matching** ("Blade Runner 2049 (Radarr)") needs the same `ListLibraryFilesAsync` port extension #507 adds. `LibraryScanHandler` still reports every file unmatched (`manager_kind`/`manager_title` are always `null`) — #505 explicitly allows this ("An unmatched file is still processable"). Wiring it up (calling it per linked connection in `LibraryScanHandler`, same as the notify call in `LibraryCleanHandler`) remains a small follow-up, not done here.
- **#508 (seeding / re-download-risk warnings)**: `Weir.Core.LibraryMode.HardlinkPolicy`/`LibraryCleanPreflight` and two new per-library settings, `clean_hardlinked_files` (default false) and `skip_if_manager_would_redownload` (default true, not yet enforced — see below), are wired into `LibraryCleanHandler` and the `library-files/clean`/`library-schedule` endpoints: a file another name still shares data with is skipped, and never queued even when the removal is confirmed, unless the library opts in. `ConfirmationRequired`'s `warnings` array is populated from the same preflight, run at request time over the selected/scanned files (a scan can be stale). The re-download-risk half (`RedownloadRiskEvaluator`/`RedownloadRiskChecker`, fully built and tested) is not called yet: it needs the manager's file id and quality profile id, which the manager title matching above does not resolve — `LibraryCleanPreflight.Evaluate` already accepts an optional risk assessment for exactly this reason, it just never receives one until that matching lands.
