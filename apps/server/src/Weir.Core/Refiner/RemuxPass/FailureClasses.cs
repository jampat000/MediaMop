...
/// <summary>
/// Why a file failed, in the terms a retry policy can act on (port of <c>refiner_failure_classes.py</c>).
/// </summary>
public static class RefinerFailureClasses
{
...
/// <summary>The automatic half of <c>refiner_requeue_service.py</c>: the library's retry policy applied to one failure.</summary>
public static class RetryPolicy
{
    /// <summary><c>REFINER_QUARANTINE_AFTER_FAILURES</c>.</summary>
    public const int QuarantineAfterFailures = 3;

    /// <summary><c>decide_retry</c>.</summary>
    public static RetryDecision DecideRetry(RefinerLibraryRecord library, string failureClass, long attemptsSoFar, DateTimeOffset now)
    {
        ArgumentNullException.ThrowIfNull(library);
        var value = PyStrings.Strip(failureClass ?? string.Empty).ToLowerInvariant();
        if (!RefinerFailureClasses.IsRetryable(value, library.RetryPreflightFailures, library.RetryExecutionFailures))
        {
        ...
        var delay = RefinerFailureClasses.BackoffSecondsForAttempt(attemptsSoFar, library.RetryBackoffSeconds);
    ...
    /// the same class.
    /// </summary>
    public static RetryDecision DecideForRecordedFailure(RefinerLibraryRecord library, string failureClass, long previousAttempts, string? previousFailureClass, DateTimeOffset now)
    {
        var value = PyStrings.Strip(failureClass ?? string.Empty).ToLowerInvariant();
    ...
    private static string NotRetryableReason(string failureClass, RefinerLibraryRecord library) => failureClass switch
    {
        RefinerFailureClasses.Preflight =>
        ...
        RefinerFailureClasses.Guardrail =>
        ...
        RefinerFailureClasses.Execution =>
