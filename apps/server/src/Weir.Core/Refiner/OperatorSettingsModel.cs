...
/// <summary>The singleton <c>refiner_operator_settings</c> row (id = 1).</summary>
public sealed record RefinerOperatorSettingsRecord
{
    ...
    public long RefinerMinInputFileSizeMb { get; init; } = 50;
...
/// <summary>
/// Weekday-CSV and HH:MM validation, and wall-clock window evaluation, shared by Refiner's Movies/TV
/// schedule pair (port of <c>weir.platform.media_managers.schedule_csv_validate</c> and
/// <c>schedule_wall_clock</c> — small enough to live with Refiner rather than wait on the media-manager port).
/// </summary>
...
/// <summary>Clamping and normalization for <c>refiner_operator_settings</c> (port of the module-level helpers
/// in <c>refiner_operator_settings_service.py</c>).</summary>
