...
/// <c>refiner_remux_rules_settings_service.rule_set_to_rules_config</c>). Deliberately the simpler of the
...
public static class RuleSetConversion
{
    ...
    public static RefinerRulesConfig ToRulesConfig(RefinerRuleSetRecord? row)
    {
        if (row is null)
        {
