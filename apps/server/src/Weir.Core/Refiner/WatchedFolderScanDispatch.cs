...
/// (port of <c>refiner_watched_folder_remux_scan_dispatch_job_kinds.py</c>).</summary>
public static class RefinerWatchedFolderScanDispatchJobKinds
{
    public const string ScanDispatch = "refiner.watched_folder.remux_scan_dispatch.v1";
}
...
/// <c>refiner_library_periodic_scan_enabled</c>/<c>refiner_scope_periodic_scan_enabled</c>).</summary>
public static class ScanDispatchScheduleGate
{
    /// <summary><c>refiner_library_periodic_scan_enabled</c>: the library's own on/off switch.</summary>
    ...
    /// the "Settings" switch an operator sees and toggles on the Refiner operator-settings screen. Python's
    /// scheduler never read this column either (only <c>library.enabled</c>), so it was as dead as the
    /// separate <c>WEIR_REFINER_WATCHED_FOLDER_REMUX_SCAN_DISPATCH_SCHEDULE_ENABLED</c> environment
    /// variable — see <c>RefinerWatchedFolderScanDispatchScheduleTask</c> in Weir.Infrastructure, which
    ...
    public static bool ScopePeriodicScanEnabled(bool movieScheduleEnabled, bool tvScheduleEnabled, string mediaScope) =>
        RefinerMediaScopes.Normalize(mediaScope) == RefinerMediaScopes.Tv ? tvScheduleEnabled : movieScheduleEnabled;
}
    ...
    bool TopLevelOnly)
{
    ...
    public static LibraryAdmissionRules For(RefinerLibraryRecord library)
    {
...
{
    public bool Eligible => Status == RefinerFileStatuses.Unprocessed;
}

/// <summary>Port of <c>refiner_file_state_service.decide_file_state</c> (pure decision; the caller supplies
...
public static class FileStateDecision
{
    ...
    {
        ArgumentNullException.ThrowIfNull(library);
        var scopeWord = library.MediaType == RefinerMediaScopes.Tv ? "TV episodes" : "Movies";

        if (!library.Enabled)
        {
            return new FileStateVerdict(RefinerFileStatuses.Disabled, $"The {library.Name} library is switched off, so Weir is leaving its files alone.");
        }

        if (!string.IsNullOrEmpty(pausedReason))
        {
            return new FileStateVerdict(RefinerFileStatuses.OutOfSchedule, pausedReason, HoldUntil: pausedUntil);
        }

        if (library.ScheduleEnabled && !inScheduleWindow)
        {
            return new FileStateVerdict(
                RefinerFileStatuses.OutOfSchedule,
        ...
        {
            return new FileStateVerdict(RefinerFileStatuses.OnHold, accessProblem);
        }

        if (!string.IsNullOrEmpty(blockedByConnection))
        {
            return new FileStateVerdict(
                RefinerFileStatuses.BlockedUpstream,
...
/// <summary>Port of <c>refiner_runner_units.resolution_class_for_dimensions</c>.</summary>
