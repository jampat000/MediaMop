...
public sealed record JobRowsPruneCounts(int Refiner, int HandoffLedger, int Activity)
{
    public int Total => Refiner + Activity;
}

/// <summary>
/// Periodic pruning of terminal job rows (port of <c>weir.platform.jobs.job_rows_retention_periodic</c>):
/// terminal <c>refiner_jobs</c> rows past <c>WEIR_JOB_ROWS_RETENTION_DAYS</c>, terminal hand-off ledger
...
public static class JobRowsRetention
{
    ...
    {
        var statuses = RefinerJobStatus.Terminal;
        ...
        return RefinerJobStore.Execute(
            connection,
            transaction,
            $"DELETE FROM refiner_jobs WHERE status IN ({string.Join(", ", names)}) AND updated_at < @cutoff",
            parameters);
    }

    /// <summary><c>_run_prune_tick</c>.</summary>
    public static Task<JobRowsPruneCounts> RunTickAsync(RefinerJobStore queue, int jobRowsRetentionDays, DateTimeOffset now, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(queue);
        return queue.InTransactionAsync(
            (connection, transaction) =>
            {
                var refiner = PruneJobRows(connection, transaction, now - TimeSpan.FromDays(jobRowsRetentionDays));
                ...
                return new JobRowsPruneCounts(refiner, ledger, activity);
            },
            cancellationToken);
    }
    ...
    {
        var value = RefinerJobStore.Scalar(connection, transaction, "SELECT activity_retention_days FROM suite_settings WHERE id = 1");
