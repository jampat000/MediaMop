...
/// <summary>
/// Periodic tick for <c>refiner.watched_folder.remux_scan_dispatch.v1</c> (port of
/// <c>refiner_watched_folder_remux_scan_dispatch_periodic_enqueue.py</c>'s asyncio loop), run as an
...
/// <list type="bullet">
/// <item>
/// <see cref="WeirOptions.RefinerWatchedFolderRemuxScanDispatchScheduleEnabled"/>
/// (<c>WEIR_REFINER_WATCHED_FOLDER_REMUX_SCAN_DISPATCH_SCHEDULE_ENABLED</c>, default on): a global kill
...
/// consult any of this — see <see cref="RefinerWatchedFolderScanDispatchEnqueue.EnqueueScanDispatchJobAsync"/>,
/// which the manual HTTP route calls directly.
/// <para>
/// <c>..._PERIODIC_ENQUEUE_REMUX_JOBS</c> (<see cref="WeirOptions.RefinerWatchedFolderRemuxScanDispatchPeriodicEnqueueRemuxJobs"/>)
...
/// </para>
/// </remarks>
public sealed class RefinerWatchedFolderScanDispatchScheduleTask : IPeriodicTask
{
    private readonly SqliteDatabase _database;
    private readonly WeirOptions _options;
    private readonly RefinerJobStore _jobStore;
    private readonly TimeProvider _time;
    private readonly ILogger<RefinerWatchedFolderScanDispatchScheduleTask> _logger;
    private readonly Dictionary<long, DateTimeOffset> _nextRunByLibrary = [];

    public RefinerWatchedFolderScanDispatchScheduleTask(
        SqliteDatabase database, WeirOptions options, RefinerJobStore jobStore, TimeProvider time, ILogger<RefinerWatchedFolderScanDispatchScheduleTask> logger)
    {
    ...
    public string Name => "refiner-watched-folder-remux-scan-dispatch-enqueue";
    ...
    {
        if (!_options.RefinerWatchedFolderRemuxScanDispatchScheduleEnabled)
        {
        ...
        // below opens its own connection through RefinerJobStore, and SQLite allows only one writer.
        ...
        foreach (var library in libraries)
        {
            ...
            try
            {
                var (inserted, skip) = await RefinerWatchedFolderScanDispatchEnqueue.TryEnqueuePeriodicAsync(
                    uow, _jobStore, library, _options.RefinerWatchedFolderRemuxScanDispatchPeriodicEnqueueRemuxJobs).ConfigureAwait(false);
                await uow.CommitAsync().ConfigureAwait(false);
    ...
    private static bool ScanDispatchScheduleGateEnabled(RefinerLibraryRecord library) =>
