...
/// <summary>Resolved folders and output-side settings for one pass (<c>RefinerPathRuntime</c>).</summary>
public sealed record RefinerPathRuntime
{
...
/// (<c>refiner_path_settings_service.py</c>), its rules (<c>rules_config_for</c>) and the rejected-file primitive
/// (<c>refiner_rejected_file_cleanup.py</c>).
/// </summary>
public static class RemuxPassPaths
{
    ...
    /// <summary>
    /// <c>resolve_media_file_under_refiner_root</c>: <c>(root / relative).resolve()</c>, only when it stays under the watched
    ...
    /// <summary><c>resolved_default_refiner_work_folder</c> / <c>resolved_default_refiner_tv_work_folder</c>.</summary>
    public static string DefaultWorkFolder(string weirHome, string mediaType) =>
        Path.Join(Resolve(weirHome), "refiner", mediaType == "tv" ? "refiner-tv-work" : "refiner-movie-work");

    /// <summary><c>effective_library_work_folder</c>: the library's own, or the per-scope default it used to use.</summary>
    public static (string WorkFolder, bool IsDefault) EffectiveWorkFolder(RefinerLibraryRecord library, string weirHome)
    {
        ...
        var legacy = scope == "tv" ? @"C:\ProgramData\Weir\refiner-tv-work" : @"C:\ProgramData\Media\refiner-movie-work";
    ...
    /// <summary>
    /// <c>resolve_refiner_path_runtime_for_library</c>: one library's folders, or the sentence saying why they cannot be used.
    /// </summary>
    ...
    /// </summary>
    public static RefinerRulesConfig RulesConfigFor(RefinerRuleSetRecord ruleSet)
    {
