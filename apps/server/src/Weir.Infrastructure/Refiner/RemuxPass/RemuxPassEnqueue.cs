...
/// <summary>The work behind <c>POST /refiner/jobs/file-remux-pass/enqueue</c> (port of <c>file_remux_pass/api.py</c>).</summary>
public static class RemuxPassEnqueue
{
    ...
    public static async Task<RefinerJob> EnqueueManualAsync(
        UnitOfWork uow,
        ...
        bool passThroughUnchanged)
    {
        ...
        if (passThroughUnchanged && await PendingJobForPathAsync(uow, relative, effectiveLibraryId).ConfigureAwait(false) is { } pending)
        {
            ...
            await uow.ExecuteAsync(
                "UPDATE refiner_jobs SET payload_json = $payload, not_before = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = $id",
                ("$payload", json),
    ...
    public static async Task<RefinerJob?> PendingJobForPathAsync(UnitOfWork uow, string relativePath, long? libraryId)
    {
        ArgumentNullException.ThrowIfNull(uow);
        ...
        var rows = await uow.QueryAsync(
            ...
            "not_before, runner_cost, priority, created_at, updated_at FROM refiner_jobs WHERE job_kind = $kind AND status = $pending ORDER BY id",
            MediaManagers.HandoffLedgerStore.ReadJob,
            ("$kind", RemuxPassOutcomes.JobKind),
            ("$pending", RefinerJobStatus.Pending)).ConfigureAwait(false);
