...
/// <summary>
/// Create Refiner libraries from what a connected media manager already knows (port of
/// <c>refiner_library_discovery.py</c>, #554).
///
...
///
/// Two rules shape everything here, and both come from what Refiner does after a successful pass — it
...
public sealed class LibraryDiscoveryService
{
        ...
        UnitOfWork uow, MediaManagerConnectionRecord connectionRow, CancellationToken cancellationToken)
    {
        ...
        if (resolved.Count == 0)
        {
            throw new RefinerDiscoveryException(
        ...
        if (port is null)
        {
            throw new RefinerDiscoveryException($"Weir does not know how to talk to a {connectionRow.Kind} connection.");
        }
        ...
        {
            throw new RefinerDiscoveryException(described.Detail ?? $"{resolved[0].Label} did not answer when asked what it manages.");
        }
    ...
    private static string UniqueName(HashSet<string> existing, string wanted)
    {
        ...
        throw new RefinerDiscoveryException($"Too many libraries already named like {PyStrings.Repr(wanted)}.");
    }

    /// <summary>
    /// <c>import_libraries</c>: create a Refiner library per selected manager library.
    ///
    ...
    /// </summary>
    public async Task<List<RefinerLibraryRecord>> ImportLibrariesAsync(
        UnitOfWork uow, MediaManagerConnectionRecord connectionRow, IReadOnlyList<string> keys, CancellationToken cancellationToken = default)
    {
        ...
        if (wanted.Count == 0)
        {
            throw new RefinerDiscoveryException("Choose at least one library to import.");
        }
        ...
        var created = new List<RefinerLibraryRecord>();
        foreach (var key in wanted)
        {
            ...
            var row = new RefinerLibraryRecord
            {
                Name = name,
                MediaType = string.IsNullOrEmpty(descriptor.MediaScope) ? RefinerMediaScopes.Movie : descriptor.MediaScope,
                DisplayOrder = order,
    ...
    public static Task<RefinerLibraryRecord> UnlinkLibraryAsync(UnitOfWork uow, RefinerLibraryRecord row) =>
