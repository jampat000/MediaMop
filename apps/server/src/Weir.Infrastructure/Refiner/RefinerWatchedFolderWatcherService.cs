...
/// <c>refiner_watched_folder_watcher.py</c>). One <see cref="FileSystemWatcher"/> per enabled library with
/// a saved watched folder and filesystem events switched on, recursive, debounced into exactly the same
/// <c>refiner.watched_folder.remux_scan_dispatch.v1</c> job the periodic timer enqueues
/// (<see cref="RefinerWatchedFolderScanDispatchEnqueue.TryEnqueueForWatcherEventAsync"/>) — this decides
...
/// </list>
/// </remarks>
public class RefinerWatchedFolderWatcherService : BackgroundService
{
    ...
    private readonly ILogger<RefinerWatchedFolderWatcherService> _logger;

    public RefinerWatchedFolderWatcherService(
        SqliteDatabase database,
        WeirOptions options,
        RefinerJobStore jobStore,
        WatcherStateStore state,
        TimeProvider time,
        ILogger<RefinerWatchedFolderWatcherService> logger)
    {
    ...
    {
        if (!_options.RefinerWatcherEnabled)
        {
        ...
        try
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                var debounce = TimeSpan.FromSeconds(Math.Clamp(_options.RefinerWatcherDebounceSeconds, 0.25, 300.0));
    ...
    {
        List<RefinerLibraryRecord> libraries;
    ...
    private async Task EnqueueScansAsync(IReadOnlyList<long> libraryIds, CancellationToken cancellationToken)
    {
        try
        {
            ...
            foreach (var libraryId in libraryIds)
            {
                ...
                var (inserted, skip) = await RefinerWatchedFolderScanDispatchEnqueue.TryEnqueueForWatcherEventAsync(
    ...
    private static bool HasWatchedFolder(RefinerLibraryRecord row) => !string.IsNullOrWhiteSpace(row.WatchedFolder);
    ...
    private sealed class LibraryWatch(RefinerLibraryRecord library, string watchedFolder, FileSystemWatcher? watcher)
    {
        public RefinerLibraryRecord Library { get; set; } = library;
