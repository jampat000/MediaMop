...
/// <c>refiner_watched_folder_remux_scan_dispatch_api.py</c> and
/// <c>schemas_watched_folder_remux_scan_dispatch_manual.py</c>; <c>refiner_jobs</c> only).</summary>
public static class RefinerWatchedFolderScanDispatchEndpoints
{
    public static IEndpointRouteBuilder MapRefinerWatchedFolderScanDispatchEndpoints(this IEndpointRouteBuilder endpoints)
    {
        endpoints.MapV1("POST", "/refiner/jobs/watched-folder-remux-scan-dispatch/enqueue", PostEnqueueAsync);
        return endpoints;
    }
    ...
    private static async Task<ApiResult> PostEnqueueAsync(ApiRequest request)
    {
        ...
        var (ok, error) = await RefinerWatchedFolderScanDispatchEnqueue.ValidatePrerequisitesAsync(
        ...
        var job = await RefinerWatchedFolderScanDispatchEnqueue.EnqueueScanDispatchJobAsync(
