...
import * as api from "../../lib/refiner/direct-play-api";
import type { DirectPlayDevices } from "../../lib/refiner/direct-play-api";
...
it("lists each device with its note and source, and saves the chosen ids", async () => {
  ...
  render(<RefinerDirectPlaySection />, { wrapper });
  ...
  expect(
    screen.queryByTestId("refiner-direct-play-customised"),
...
it("says when the list comes from the operator's own file", async () => {
  ...
  expect(
    await screen.findByTestId("refiner-direct-play-customised"),
