...
import type { RefinerRuleSetWrite } from "../../lib/refiner/libraries-api";
import {
  useRefinerLibrariesQuery,
  useRefinerRulesPreview,
} from "../../lib/refiner/libraries-queries";
import type {
  RefinerRulesPreviewResult,
  RefinerRulesPreviewTrack,
} from "../../lib/refiner/rules-preview-api";
import {
  mmActionButtonClass,
...
const TRACK_TYPE_LABELS: Record<RefinerRulesPreviewTrack["type"], string> = {
  video: "Video",
  audio: "Audio",
  subtitle: "Subtitle",
};

function TrackRow({ track }: { track: RefinerRulesPreviewTrack }) {
...
function PreviewResults({ result }: { result: RefinerRulesPreviewResult }) {
...
export function RefinerRulesPreviewPanel({
  ...
  const [result, setResult] = useState<RefinerRulesPreviewResult | null>(null);
