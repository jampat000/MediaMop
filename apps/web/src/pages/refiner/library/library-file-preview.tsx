...
import { previewRefinerRules } from "../../../lib/refiner/rules-preview-api";
import { refinerStreamLanguageLabel } from "../../../lib/refiner/stream-language-options";
import { formatBytes } from "../../../lib/refiner/library-api";
...
export function LibraryFilePreview({
  ...
  const preview = useQuery({
    queryKey: ["refiner", "library-file-preview", libraryId, path],
    queryFn: () => previewRefinerRules({ libraryId, absolutePath: path }),
          ...
          <tbody>
            {result.tracks.map((track) => (
              ...
              >
                ...
                <td className="py-1.5">
                  {refinerStreamLanguageLabel(track.language)}
                </td>
                <td className="py-1.5 text-[var(--mm-text2)]">
                  {track.codec}
