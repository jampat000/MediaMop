...
import type {
  RefinerFileTrack,
  RefinerFileTracks,
  RefinerManualPlanChoice,
} from "../../lib/refiner/files-api";

type SelectableType = "video" | "audio" | "subtitle";

function isSelectable(type: RefinerFileTrack["type"]): type is SelectableType {
...
function initialRowState(track: RefinerFileTrack): RowState {
  return {
...
function trackLabel(track: RefinerFileTrack): string {
  const bits: string[] = [];
...
export interface ChooseTracksPanelProps {
  open: boolean;
  fileName: string;
  tracks: RefinerFileTracks | undefined;
  loading: boolean;
  loadError: string | null;
  submitting: boolean;
  submitError: string | null;
  onClose: () => void;
  onSubmit: (choice: RefinerManualPlanChoice) => void;
}
...
export function ChooseTracksPanel({
  ...
  const setKeep = (track: RefinerFileTrack, keep: boolean) => {
    setRows((previous) => ({
      ...previous,
  ...
  const keptTracks = order
    ...
    .filter((track): track is RefinerFileTrack => track !== undefined);
