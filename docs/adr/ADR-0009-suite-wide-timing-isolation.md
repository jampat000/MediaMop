...
- **`refiner.file.remux_pass.v1`** — manual enqueue only: per-file ffprobe + remux plan (+ optional ffmpeg when ``dry_run`` is false); **no** periodic schedule or shared timing row with other Refiner families or Fetcher.
...
| Refiner durable families (supplied payload evaluation, candidate gate, file remux pass) | Yes | Separate job kinds, handlers, and enqueue paths; supplied payload evaluation has its own optional schedule env + interval only for that family; candidate gate and file remux pass are manual-only in this pass. No shared last-run or cooldown across those families. |
