...
- Fixed scheduled relaunch task handling in the Windows updater:
  - relaunch task is now a singleton (`Weir-Relaunch`) instead of creating unbounded per-attempt task names
  - stale `Weir-Relaunch-*` scheduled tasks are purged before relaunch
  - stale `relaunch-Weir-Relaunch*.cmd` launcher scripts are purged before relaunch
...
[Full Changelog](https://github.com/jampat000/Weir/compare/v2.1.12...v2.1.13)
