...
remux temp file lives (``{stem}.refiner.XXXXXXXX{suffix}``), so killing the server during that
...
def test_a_crash_mid_remux_leaves_no_temp_output_and_the_job_still_finishes(
    ...
    operator_note = folders.work / "film.refiner.notes.txt"
    ...
    wait_until(
        lambda: list(folders.work.glob("film.refiner.*.mkv")),
        timeout_s=60,
        what="the remux to start writing its temp output in the work folder",
    )

    server.kill()
    assert list(folders.work.glob("film.refiner.*.mkv")), "the kill must leave the half-written temp output behind"
    ...
    assert not list(folders.work.glob("film.refiner.*.mkv")), "the interrupted job's temp output must be removed"
