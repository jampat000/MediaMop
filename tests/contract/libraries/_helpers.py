"""Shared helpers for the Processing contract tests."""
...
REMUX_PASS_JOB_KIND = "processing.file.remux_pass.v1"
REMUX_PASS_COMPLETED_EVENT = "processing.file_remux_pass_completed"
...
def delete_with_body_csrf(c: WeirClient, path: str) -> httpx.Response:
    """DELETE carrying ``csrf_token`` in a JSON body, the shape the Processing delete routes take."""
...
def first_library_id(conn: Any) -> int:
    return int(seed.scalar(conn, "SELECT id FROM libraries ORDER BY id LIMIT 1"))
