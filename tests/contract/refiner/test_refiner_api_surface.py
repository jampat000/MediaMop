"""Port of the retired Python backend's tests/test_refiner_api_surface.py."""
...
def test_maintenance_state_needs_a_session(client) -> None:
    assert client.get(f"{API}/refiner/maintenance").status_code == 401


def test_maintenance_state_lists_every_promoted_family(admin) -> None:
    body = admin.get(f"{API}/refiner/maintenance").json()
...
def test_triggering_maintenance_without_a_csrf_token_is_refused(admin) -> None:
    response = admin.post(
        f"{API}/refiner/maintenance/run", json={"family": "work_temp_stale_sweep", "media_scope": "movie"}
    )
...
def test_triggering_maintenance_queues_a_run(admin) -> None:
    response = admin.post_csrf(
        f"{API}/refiner/maintenance/run", {"family": "work_temp_stale_sweep", "media_scope": "movie"}
    )
...
def test_an_unknown_family_is_refused(admin) -> None:
    response = admin.post_csrf(
        f"{API}/refiner/maintenance/run", {"family": "something_invented", "media_scope": "movie"}
    )
    assert response.status_code == 422


def test_the_hardware_report_needs_a_session_and_then_answers(client) -> None:
    assert client.get(f"{API}/refiner/hardware").status_code == 401

    client.ensure_admin()
    body = client.get(f"{API}/refiner/hardware").json()
...
def test_the_metadata_provider_never_returns_the_key(admin) -> None:
    ...
    assert admin.get(f"{API}/refiner/metadata-provider").status_code == 200

    saved = admin.put_csrf(
        f"{API}/refiner/metadata-provider",
    ...
    assert "a-secret-key" not in admin.get(f"{API}/refiner/metadata-provider").text
