...
``RefinerFileOut.status`` (``schemas_refiner_files.py``) does not list ``passed_through`` or
``rejected`` even though #471 added both as file statuses, so pydantic response validation raises
...
because of this bug; see the comment there.
"""
