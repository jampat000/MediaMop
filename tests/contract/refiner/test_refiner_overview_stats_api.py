...
PATH = f"{API}/refiner/overview-stats"
...
def test_refiner_overview_stats_requires_auth(client) -> None:
    r = client.get(PATH)
    assert r.status_code == 401


def test_refiner_overview_stats_shape(admin) -> None:
    r = admin.get(PATH)
...
def test_refiner_overview_stats_aggregates_remux_savings(server_factory, client_factory) -> None:
    sut = server_factory()
    home = Path(sut.home)
...
def test_refiner_overview_stats_excludes_non_finalized_successes(server_factory, client_factory) -> None:
    sut = server_factory()
    home = Path(sut.home)
