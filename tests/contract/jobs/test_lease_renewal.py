...
On .NET, the lease-length knob and the fix both exist: ``WEIR_REFINER_JOB_LEASE_SECONDS``
(``WeirOptions.RefinerJobLeaseSeconds``, clamped to 30..86400s) shortens the lease, and
``RefinerJobProcessor.RunHandlerWithLeaseRenewalAsync`` runs a heartbeat that renews it roughly every
...
there, though: no ``IJobHandler`` is registered for ``refiner.file.remux_pass.v1`` anywhere in this
...
NO_END_TO_END_REMUX_YET = (
    "python has no WEIR_REFINER_JOB_LEASE_SECONDS override (see module docstring), and .NET has the "
    "lease knob and renewal heartbeat but no IJobHandler for refiner.file.remux_pass.v1 yet, so no "
...
def test_a_job_longer_than_its_lease_is_never_claimed_twice(
    ...
    # must still only ever be claimed by one of them. 30s is the minimum WEIR_REFINER_JOB_LEASE_SECONDS
    # accepts (WeirOptionsLoader.ClampRefinerJobLeaseSeconds); the heartbeat renews it about every 10s,
    ...
    server = server_factory(
        env={
            **h.working_env(fake_ffmpeg),
            ...
            "WEIR_REFINER_JOB_LEASE_SECONDS": "30",
        }
    )
