"""Contract port of the retired Python backend's tests/test_refiner_file_remux_pass_manual_enqueue_api.py."""
...
ENQUEUE = f"{API}/refiner/jobs/file-remux-pass/enqueue"


def test_refiner_file_remux_pass_enqueue_writes_live_payload(admin: WeirClient, tmp_path: Path) -> None:
...
def test_refiner_file_remux_pass_enqueue_rejects_missing_watched_folder(admin: WeirClient, tmp_path: Path) -> None:
...
def test_pass_through_converts_an_existing_pending_job_instead_of_duplicating_it(
    ...
    with seed.stopped(server) as conn:
        conn.execute("DELETE FROM refiner_jobs WHERE id != ? AND job_kind = ?", (job_id, REMUX))
        raw = seed.scalar(conn, "SELECT payload_json FROM refiner_jobs WHERE id = ?", (job_id,))
        queued_payload = json.loads(raw or "{}")
        queued_payload["origin"] = {"source_key": "radarr", "handoff_id": "handoff-1"}
        conn.execute(
            "UPDATE refiner_jobs SET payload_json = ? WHERE id = ?",
