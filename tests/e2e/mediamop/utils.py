...
def clear_auth_tables_for_home(home: str) -> None:
    ...
    test.  Singleton rows (RefinerPathSettingsRow) are *reset* rather than
    deleted because the backend raises RuntimeError when the migration-seeded
    row with id=1 is missing.  Per-instance rows (PrunerServerInstance and
    their children) are deleted outright because they are not singletons.
    """
